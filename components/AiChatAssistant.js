// /components/AiChatAssistant.js - v64 (功能与视觉终极版)
...
const AiTtsButtonModified = ({ text, ttsSettings }) => {
    ...
    // 匹配带数字声调的拼音，例如 ni3 hao3
    const pinyinRegex = /\b[a-zA-ZüÜ]+[1-5]\b/g;
    // 匹配 Emoji
    const emojiRegex = /(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/g;

    const modifiedText = textToFilter
        .replace(bracketRegex, '') // 移除所有括号及其内容
        .replace(pinyinRegex, '')   // 移除带音调数字的拼音
        .replace(emojiRegex, '');   // 移除表情
    
    return <AiTtsButton text={modifiedText.trim()} ttsSettings={ttsSettings} />;
};

// [核心修改] 打字效果组件 (无变化，但现在会稳定触发)
...
const MessageBubble = ({ msg, settings, isLastAiMessage, onRegenerate, onTypingComplete, onTypingUpdate }) => {
    ...
    const avatarClass = "w-8 h-8 rounded-full shrink-0 shadow-sm"; // 移除了边框
            ...
            <div className={`p-3 rounded-2xl text-left flex flex-col transition-shadow duration-300 ${isUser ? userBubbleClass : aiBubbleClass}`} style={{ maxWidth: '85%' }}>
                ...
                {!isUser && msg.content && !msg.isTyping && (
                    <div className="flex items-center gap-2 mt-2 -mb-1 text-gray-500">
                        ...
                        {isLastAiMessage && (
                            <button onClick={(e) => { e.stopPropagation(); onRegenerate(); }} className="p-2 rounded-full hover:bg-black/10 transition-colors" title="重新生成"><i className="fas fa-sync-alt"></i></button>
                        )}
                    </div>
                )}
            </div>
...
const ModelSelector = ({ settings, onSelect, onClose }) => (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 flex flex-col p-4 animate-fade-in" onClick={onClose}>
        <div className="w-full max-w-md m-auto bg-gray-100 rounded-xl shadow-lg flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-200 text-center relative">
                <h3 className="text-xl font-bold">切换模型</h3>
                <button onClick={onClose} className="absolute top-2 right-2 p-2 rounded-full hover:bg-gray-200"><i className="fas fa-times"></i></button>
            </div>
            <div className="p-2 overflow-y-auto max-h-[60vh]">
                {(settings.chatModels || []).map(m => (
                    <button key={m.id} type="button" onClick={() => { onSelect(m.value); onClose(); }} className={`w-full text-left px-4 py-3 rounded-lg hover:bg-primary/10 transition-colors ${settings.selectedModel === m.value ? 'text-primary bg-primary/10' : ''}`}>
...
const AssistantSelector = ({ prompts, settings, onSelect, onClose }) => (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 flex flex-col p-4 animate-fade-in" onClick={onClose}>
        <div className="w-full max-w-2xl m-auto bg-gray-100 rounded-xl shadow-lg flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-200 text-center relative">
                <h3 className="text-xl font-bold">更换助理</h3>
                <button onClick={onClose} className="absolute top-2 right-2 p-2 rounded-full hover:bg-gray-200"><i className="fas fa-times"></i></button>
            </div>
            <div className="p-4 grid grid-cols-2 sm:grid-cols-3 gap-4 overflow-y-auto max-h-[60vh]">
                {(prompts || []).map(p => (
                    <button key={p.id} onClick={() => onSelect(p.id)} className={`flex flex-col items-center p-4 rounded-lg border-2 transition-all ${settings.currentPromptId === p.id ? 'border-primary bg-primary/10' : 'border-transparent hover:bg-white/60'}`}>
...
const AiChatAssistant = ({ onClose }) => {
    ...
    // [核心修改] 模型选择Bug修复: 当选择新助理时，同步更新当前使用的模型
    const handleSelectAssistant = (promptId) => {
        const selectedPrompt = settings.prompts.find(p => p.id === promptId);
        if (selectedPrompt) {
            setSettings(s => ({
                ...s,
                currentPromptId: promptId,
                selectedModel: selectedPrompt.model || s.selectedModel
            }));
        }
    ...
    return (
        <div className="w-full h-full flex flex-col bg-gray-100 text-gray-800">
            <div className="absolute inset-0 bg-cover bg-center opacity-30" style={{ backgroundImage: `url('${settings.chatBackgroundUrl.startsWith('blob:') ? settings.chatBackgroundUrl : convertGitHubUrl(settings.chatBackgroundUrl)}')`}}></div>
            ...
            <div className="relative flex flex-1 min-h-0">
                ...
                {showAssistantSelector && <AssistantSelector prompts={settings.prompts} settings={settings} onSelect={handleSelectAssistant} onClose={() => setShowAssistantSelector(false)} />}
