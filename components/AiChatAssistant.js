// /components/AiChatAssistant.js - v66.1 (TTS Fix for Custom Endpoints)
...
const AiTtsButton = ({ text, ttsSettings }) => {
    ...
    const startPlayback = async () => {
        ...
        try {
            ...
            // 方案A: 如果是OpenAI兼容接口 (例如您的 https://t.leftsite.cn)
            ...
            // 方案B: 如果是Microsoft Azure接口 (默认)
            else {
                // 注意：使用微软服务也需要API Key，需要在设置中填写
                if (!config.apiKey) throw new Error("Microsoft Azure TTS API 密钥未在设置中填写。");

                const region = 'eastus'; // 微软服务区域，可按需修改
