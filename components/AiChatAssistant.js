// /components/AiChatAssistant.js - v58 (根据用户需求修改版)
...
// 修改: ChatSidebar，移除三点菜单，直接显示重命名和删除按钮
const ChatSidebar = ({ isOpen, conversations, currentId, onSelect, onNew, onDelete, onRename, prompts, settings }) => {
        ...
        >
            <div className="flex-grow truncate" onDoubleClick={(e) => { e.stopPropagation(); handleRename(conv.id, conv.title); }}>
                {editingId === conv.id ? (
                    <input
                        ...
                        className="w-full bg-transparent p-0 border-b dark:border-gray-500"
                        autoFocus
                    />
                ) : (
            ...
            <div className={`flex items-center shrink-0 space-x-1 transition-opacity ${currentId === conv.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                <button
                    ...
                    title="重命名"
                >
                    <i className="fas fa-pen w-3 h-3"></i>
                </button>
                <button
                    onClick={(e) => { e.stopPropagation(); if (window.confirm('确定删除此对话吗？')) onDelete(conv.id); }}
    ...
    return (
        <div className={`h-full bg-gray-50/80 dark:bg-gray-800/80 backdrop-blur-sm flex flex-col border-r dark:border-gray-700 transition-all duration-300 ${isOpen ? 'w-44 p-2' : 'w-0 p-0'} overflow-hidden`}>
...
const ModelManager = ({ models, onBack, onChange, onAdd, onDelete }) => { const swipeHandlers = useSimpleSwipe({ onSwipeLeft: onBack }); return ( <div {...swipeHandlers} className="fixed inset-0 bg-white dark:bg-gray-800 p-6 flex flex-col z-[9999] animate-fade-in"> <div className="flex items-center justify-between mb-4 shrink-0"> <button onClick={onBack} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"><i className="fas fa-arrow-left"></i></button> <h3 className="text-2xl font-bold">模型管理</h3> <div className="w-8"></div> </div> <div className="flex-grow overflow-y-auto pr-2 space-y-3"> {(models || []).map(m => ( <div key={m.id} className="p-3 bg-gray-50 dark:bg-gray-900/50 rounded-md border dark:border-gray-700 space-y-2"> <div className="flex items-center justify-between"> <input type="text" value={m.name} onChange={(e) => onChange(m.id, 'name', e.target.value)} placeholder="模型显示名称" className="font-semibold bg-transparent w-full text-lg" /> <button onClick={() => onDelete(m.id)} className="p-2 ml-2 text-sm text-red-500 rounded-full hover:bg-red-500/10"><i className="fas fa-trash"></i></button> </div> <div className="grid grid-cols-2 gap-2 text-sm"> <div> <label className="text-xs font-medium">模型值 (Value)</label> <input type="text" value={m.value} onChange={(e) => onChange(m.id, 'value', e.target.value)} placeholder="例如: gemini-1.5-pro-latest" className="w-full mt-1 px-2 py-1 bg-white dark:bg-gray-800 border rounded-md text-xs" /> </div> <div> <label className="text-xs font-medium">最大上下文 (Tokens)</label> <input type="number" value={m.maxContextTokens} onChange={(e) => onChange(m.id, 'maxContextTokens', parseInt(e.target.value, 10) || 0)} placeholder="例如: 8192" className="w-full mt-1 px-2 py-1 bg-white dark:bg-gray-800 border rounded-md text-xs" /> </div> </div> </div> ))} </div> <button onClick={onAdd} className="w-full mt-4 py-2 bg-blue-500 text-white rounded-md shrink-0"><i className="fas fa-plus mr-2"></i>添加新模型</button> </div> ); };
const ApiKeyManager = ({ apiKeys, activeApiKeyId, onBack, onChange, onAdd, onDelete, onSetActive }) => { const swipeHandlers = useSimpleSwipe({ onSwipeLeft: onBack }); return ( <div {...swipeHandlers} className="fixed inset-0 bg-white dark:bg-gray-800 p-6 flex flex-col z-[9999] animate-fade-in"> <div className="flex items-center justify-between mb-4 shrink-0"> <button onClick={onBack} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"><i className="fas fa-arrow-left"></i></button> <h3 className="text-2xl font-bold">API 密钥管理</h3> <div className="w-8"></div> </div> <div className="flex-grow overflow-y-auto pr-2 space-y-3"> {(apiKeys || []).map(k => ( <div key={k.id} className={`p-3 rounded-md border-2 ${activeApiKeyId === k.id ? 'border-primary bg-primary/10' : 'border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50'}`}> <div className="flex items-center justify-between mb-2"> <select value={k.provider} onChange={(e) => onChange(k.id, 'provider', e.target.value)} className="font-semibold bg-transparent text-lg"> <option value="gemini">Google Gemini</option> <option value="openai">OpenAI 兼容</option> </select> <div className="flex items-center gap-2"> <button onClick={() => onSetActive(k.id)} disabled={activeApiKeyId === k.id} className="px-2 py-1 text-xs bg-green-500 text-white rounded disabled:bg-gray-400">设为当前</button> <button onClick={() => onDelete(k.id)} className="p-2 text-sm text-red-500 rounded-full hover:bg-red-500/10"><i className="fas fa-trash"></i></button> </div> </div> {k.provider === 'openai' && ( <div> <label className="text-xs font-medium">API 接口地址 (URL)</label> <input type="text" value={k.url || ''} onChange={(e) => onChange(k.id, 'url', e.target.value)} placeholder="例如: https://api.openai.com/v1" className="w-full mt-1 px-2 py-1 bg-white dark:bg-gray-800 border rounded-md text-xs" /> </div> )} <div className="mt-2"> <label className="text-xs font-medium">API 密钥 (Key)</label> <input type="password" value={k.key} onChange={(e) => onChange(k.id, 'key', e.target.value)} placeholder="请输入密钥" className="w-full mt-1 px-2 py-1 bg-white dark:bg-gray-800 border rounded-md text-xs" /> </div> </div> ))} </div> <button onClick={onAdd} className="w-full mt-4 py-2 bg-indigo-500 text-white rounded-md shrink-0"><i className="fas fa-plus mr-2"></i>添加新密钥</button> </div> ); };
...
const AiChatAssistant = ({ onClose }) => {
    ...
    const handleImageSelection = (event) => {
        ...
        const newImages = files.slice(0, 4 - selectedImages.length); // 限制最多4张图片
        ...
        event.target.value = null; // 清空input值, 方便再次选择相同文件
    };
    ...
    const removeSelectedImage = (index) => {
        ...
        if (imageToRemove) {
            URL.revokeObjectURL(imageToRemove.previewUrl); // 释放内存
        }
                    ...
                    <footer className="flex-shrink-0 px-4 pt-2 pb-safe bg-gradient-to-t from-gray-800/95 via-gray-800/80 to-transparent z-10">
                        ...
                        {selectedImages.length > 0 && (
                            <div className="max-w-2xl mx-auto mb-2 px-2">
                                <div className="flex items-center gap-2 overflow-x-auto p-1 bg-gray-900/50 rounded-lg">
                        ...
                        <form onSubmit={(e)=>{e.preventDefault();handleSubmit(false)}} className="flex items-end w-full max-w-2xl mx-auto p-2 bg-white dark:bg-gray-900/80 backdrop-blur-sm rounded-3xl shadow-lg border border-gray-200 dark:border-gray-700 transition-all duration-300 ease-in-out hover:shadow-xl focus-within:shadow-xl">
                            ...
                            {/* --- 新增: 图片和相机按钮 --- */}
                             <div className="flex items-center flex-shrink-0 mr-1">
                                <button type="button" onClick={triggerImageInput} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700" title="选择图片">
                                    <i className="fas fa-image text-xl text-gray-400"></i>
                                </button>
                                <button type="button" onClick={triggerCameraInput} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700" title="拍照">
