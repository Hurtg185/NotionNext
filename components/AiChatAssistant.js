// /components/AiChatAssistant.js - v50 (最终稳定版 - 采用“报名课程”激活界面，移除动画，修复所有Bug)
            ...
            <div className={containerClasses} style={containerStyle}>
                <div className="w-full h-full flex flex-col items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
                    <div className="w-full max-w-sm bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 flex flex-col items-center">
                        ...
                        {activationError && <p className="text-red-500 text-sm mb-4">{activationError}</p>}
                        <form onSubmit={handleActivate} className="space-y-4 w-full">
                            <input type="text" value={activationKey} onChange={(e) => setActivationKey(e.target.value)} placeholder="在此输入您的激活码" className="w-full px-4 py-3 text-center bg-gray-100 dark:bg-gray-700 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary"/>
                    ...
                    {isFullScreenMode && <button onClick={onClose} className="mt-4 px-6 py-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 bg-white/50 dark:bg-gray-700/50 rounded-full shadow-md transition-all duration-300 hover:scale-105">返回网站</button>}
                </div>
            </div>
        );
    }
