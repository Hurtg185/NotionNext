// /components/AiChatAssistant.js - v41 (高端暗黑渐变风激活界面 + 最终Bug修复)
    ...
    return (
        <div className={`flex items-end gap-2.5 my-2 animate-fade-in-up ${isUser ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`p-3 rounded-2xl text-left flex flex-col ${isUser ? userBubbleClass : aiBubbleClass}`} style={{ maxWidth: '85%' }}>
                ...
                {!isUser && msg.content && ( <div className="flex items-center gap-2 mt-2 -mb-1 text-gray-500 dark:text-gray-400"> <AiTtsButton text={msg.content} ttsSettings={settings} /> <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(msg.content); }} className="p-2 rounded-full hover:bg-black/10 dark:hover:bg-white/10" title="复制"><i className="fas fa-copy"></i></button> {isLastAiMessage && ( <button onClick={(e) => { e.stopPropagation(); onRegenerate(); }} className="p-2 rounded-full hover:bg-black/10 dark:hover:bg-white/10" title="重新生成"><i className="fas fa-sync-alt"></i></button> )} </div> )}
            </div>
...
const ChatSidebar = ({ isOpen, conversations, currentId, onSelect, onNew, onDelete, onRename, prompts, settings }) => {
    ...
    const groupedConversations = useMemo(() => {
        ...
        const uncategorized = [];
        (conversations || []).forEach(conv => { // 添加对 conversations 的安全检查
            const promptId = conv.promptId;
            const prompt = (prompts || []).find(p => p.id === promptId); // 添加对 prompts 的安全检查
            if (prompt) {
    ...
    }, [conversations, prompts, settings.aiAvatarUrl]); // 添加 settings.aiAvatarUrl 依赖，以防默认头像变化
    ...
    const renderConversationItem = (conv) => (
        <div key={conv.id} className={`group flex items-center p-2 rounded-md cursor-pointer ${currentId === conv.id ? 'bg-primary/20' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`} onClick={(e) => { e.stopPropagation(); onSelect(conv.id); setActiveMenu(null); }}>
            ...
            <div className="relative shrink-0">
                <button onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === conv.id ? null : conv.id); }} className={`p-1 rounded-full ${currentId === conv.id || activeMenu === conv.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
...
const CHAT_MODELS_LIST = [ // 重命名为 CHAT_MODELS_LIST 避免与 SettingsModal 内部的 CHAT_MODELS 变量冲突
...
const AiChatAssistant = ({ onClose, isFullScreenMode = false }) => {
            ...
            try {
                ...
                if (!activated) {
                    ...
                    if (trialResponse.ok && trialData.success) {
                        ...
                        const expiry = new Date(Date.now() + (trialData.durationDays || 7) * 24 * 60 * 60 * 1000); setTrialExpiryInfo(`试用中，有效期至: ${expiry.toLocaleDateString()} ${expiry.toLocaleTimeString()}`);
                    } else {
    ...
    if (activationState !== 'activated') {
        return (
            <div className={containerClasses} style={containerStyle}> {/* 这里使用外部传入的 containerClasses 和 containerStyle */}
                    ...
                    <div className="relative w-full max-w-md overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 to-black border border-gray-700/50 shadow-2xl animate-scale-in">
                        ...
                        <div className="p-8">
                            ...
                            <div className="flex justify-center mb-6">
                                <div className="w-16 h-16 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 p-1 shadow-lg">
                            ...
                            <form onSubmit={handleActivate} className="space-y-6">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder="输入您的激活码"
                                        className="w-full pl-10 pr-4 py-4 bg-gray-800/50 border border-gray-700/50 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-transparent transition-all duration-300"
                                    />
                                </div>
                                
                                {activationError && (
                                    <div className="p-3 bg-red-900/30 border border-red-700/50 rounded-lg text-red-300 text-sm flex items-center animate-shake">
                                ...
                                <button
                                    ...
                                    className="w-full py-4 px-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-500 hover:to-purple-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transition-all duration-300 transform hover:-translate-y-0.5 shadow-lg"
                                >
                                    {isActivating ? (
                                        <>
                            ...
                            <div className="mt-6 text-center">
                                <p className="text-gray-500 text-sm">
                                    ...
                                    <a 
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            // TODO: 这里可以添加跳转到您的Facebook主页或联系方式模态框的逻辑
                                            // 例如: window.open("https://www.facebook.com/share/1FSxz6cm2Z", "_blank");
                                            // 或者打开一个包含所有联系方式的模态框
                                        }}
                                    >
                                        获取帮助
                        ...
                        {/* 底部装饰 */}
                        <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 opacity-50"></div>
                    </div>
                </div>
            </div>
        );
