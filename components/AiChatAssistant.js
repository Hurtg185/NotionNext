// /components/AiChatAssistant.js - v51 (最终修复版 - 背景图片激活界面 + 所有功能及修复)
    ...
    if (activationState !== 'activated') {
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-lg animate-fade-in">
                <div className="w-full max-w-sm bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 flex flex-col items-center animate-scale-in">
                                ...
                                <div ref={optionsContainerRef} className="relative">
                                    ...
                                    {showMoreMenu && (
                                        <div className="absolute bottom-full mb-2 w-56 bg-white dark:bg-gray-900 rounded-lg shadow-xl border dark:border-gray-700 overflow-hidden z-20">
                                            ...
                                            <button type="button" onClick={() => { setShowAssistantSelector(true); setShowMoreMenu(false); }} className="w-full flex justify-between items-center text-left px-4 py-3 text-sm hover:bg-primary/10"> <span className="flex items-center"><i className="fas fa-user-astronaut w-6 mr-2"></i>更换助理</span> <span className="text-xs text-gray-500 truncate max-w-[100px]">{safeGetCurrentPrompt?.name}</span> </button>
