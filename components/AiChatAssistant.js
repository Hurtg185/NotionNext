// /components/AiChatAssistant.js - v66.6 (Standalone with UI-based API Manager & Voice Instructions)
...
const AiTtsButton = ({ text, ttsSettings }) => {
    ...
    const stopPlayback = useCallback(() => { if (audioRef.current) { audioRef.current.pause(); audioRef.current.src = ''; audioRef.current = null; } if (abortControllerRef.current) { abortControllerRef.current.abort(); } setIsPlaying(false); }, []);
    const startPlayback = async () => {
        ...
        try {
            const activeApiId = ttsSettings.activeTtsApiId || 'microsoft';
            const rate = ttsSettings.ttsRate || 0;
            const pitch = ttsSettings.ttsPitch || 0;
            let response;
            
            if (activeApiId === 'microsoft') {
                ...
                response = await fetch(url, { method: 'POST', headers: { 'Ocp-Apim-Subscription-Key': config.apiKey, 'Content-Type': 'application/ssml+xml', 'X-Microsoft-OutputFormat': 'audio-16khz-128kbitrate-mono-mp3', 'User-Agent': 'AiChatAssistant' }, body: ssml, signal: abortControllerRef.current.signal });
            } else { // Custom OpenAI-compatible API
                const customApi = (ttsSettings.customTtsApis || []).find(api => api.id === activeApiId);
                if (!customApi || !customApi.url) throw new Error("选择的自定义API配置不完整。");
                
                const url = customApi.url.replace(/\/$/, '');
                const headers = { 'Content-Type': 'application/json' };
                if (customApi.key) headers['Authorization'] = `Bearer ${customApi.key}`;
                
                const body = {
                    model: customApi.model || 'tts-1',
                    input: cleanedText,
                    voice: customApi.voice || 'alloy',
                    speed: 1.0 + (rate / 100),
                };

                if (ttsSettings.ttsInstructions) {
                    body.instructions = ttsSettings.ttsInstructions;
                }
                
                response = await fetch(url, { method: 'POST', headers, body: JSON.stringify(body), signal: abortControllerRef.current.signal });
            }
...
const ChatSidebar = ({ isOpen, conversations, currentId, onSelect, onNew, onDelete, onRename, prompts, settings }) => {
    ...
    const renderConversationItem = (conv) => (
        <div key={conv.id} className={`group flex items-center p-2 rounded-md cursor-pointer transition-all duration-200 ${currentId === conv.id ? 'bg-blue-500/10' : 'hover:bg-gray-200/50'}`} onClick={() => onSelect(conv.id)}>
            <div className="flex-grow truncate" onDoubleClick={(e) => { e.stopPropagation(); handleRename(conv.id, conv.title); }}>
                {editingId === conv.id ? ( <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} onBlur={() => handleSaveRename(conv.id)} onKeyDown={(e) => e.key === 'Enter' && handleSaveRename(conv.id)} className="w-full bg-transparent p-0 border-b border-gray-400" autoFocus /> ) : ( <span className={`text-sm ${currentId === conv.id ? 'text-blue-600 font-semibold' : ''}`}>{conv.title}</span> )}
            </div>
    ...
    return (
        <div className={`h-full bg-gray-100/90 backdrop-blur-md flex flex-col transition-all duration-300 z-30 ${isOpen ? 'w-60 p-3 shadow-[10px_0px_20px_rgba(0,0,0,0.1)]' : 'w-0 p-0'} overflow-hidden`}>
             <button onClick={onNew} className="flex items-center justify-center w-full px-4 py-2 mb-3 font-semibold text-gray-800 bg-white rounded-full shadow-lg shadow-gray-300/20 hover:bg-gray-50 active:scale-95 transition-all duration-200 border border-gray-200"> <i className="fas fa-plus mr-2"></i> 新对话 </button>
...
const TtsApiManager = ({ customTtsApis, onSave }) => {
    const [apis, setApis] = useState(customTtsApis || []);
    const [editingApi, setEditingApi] = useState(null);

    const handleSave = (apiToSave) => {
        let newApis;
        if (apiToSave.id) {
            newApis = apis.map(api => api.id === apiToSave.id ? apiToSave : api);
        } else {
            newApis = [...apis, { ...apiToSave, id: generateSimpleId('tts-api') }];
        }
        setApis(newApis);
        onSave(newApis);
        setEditingApi(null);
    ...
    const handleDelete = (idToDelete) => {
        ...
        const newApis = apis.filter(api => api.id !== idToDelete);
        setApis(newApis);
        onSave(newApis);
    };
    ...
    if (editingApi) {
        return (
            <div className="space-y-4">
                <h4 className="text-xl font-bold">{editingApi.id ? '编辑' : '添加'}语音 API</h4>
                <div><label className="block text-sm font-medium">API 名称</label><input type="text" value={editingApi.name} onChange={e => setEditingApi({...editingApi, name: e.target.value})} className="w-full mt-1 px-3 py-2 bg-gray-100 border rounded-md" placeholder="例如: 我的免费TTS"/></div>
                <div><label className="block text-sm font-medium">API 格式</label><select value={editingApi.format} onChange={e => setEditingApi({...editingApi, format: e.target.value})} className="w-full mt-1 px-3 py-2 bg-gray-100 border rounded-md"><option value="openai">OpenAI 格式</option></select></div>
                <div><label className="block text-sm font-medium">API 端点 URL</label><input type="text" value={editingApi.url} onChange={e => setEditingApi({...editingApi, url: e.target.value})} className="w-full mt-1 px-3 py-2 bg-gray-100 border rounded-md" placeholder="https://.../v1/audio/speech"/></div>
                <div><label className="block text-sm font-medium">API 密钥 (可选)</label><input type="password" value={editingApi.key} onChange={e => setEditingApi({...editingApi, key: e.target.value})} className="w-full mt-1 px-3 py-2 bg-gray-100 border rounded-md" placeholder="如果需要，填入 Bearer Token"/></div>
                <div><label className="block text-sm font-medium">模型 (可选)</label><input type="text" value={editingApi.model} onChange={e => setEditingApi({...editingApi, model: e.target.value})} className="w-full mt-1 px-3 py-2 bg-gray-100 border rounded-md" placeholder="例如: tts-1"/></div>
                <div><label className="block text-sm font-medium">发音人 (可选)</label><input type="text" value={editingApi.voice} onChange={e => setEditingApi({...editingApi, voice: e.target.value})} className="w-full mt-1 px-3 py-2 bg-gray-100 border rounded-md" placeholder="例如: alloy"/></div>
                <div className="flex justify-end gap-3"><button onClick={() => setEditingApi(null)} className="px-4 py-2 bg-gray-200 rounded-md">取消</button><button onClick={() => handleSave(editingApi)} className="px-4 py-2 bg-blue-600 text-white rounded-md">保存API</button></div>
            </div>
        );
    }
    ...
    return (
        <div>
            {(apis || []).map(api => (
                <div key={api.id} className="p-3 mb-2 bg-gray-50 rounded-md border flex justify-between items-center">
                    <div>
                        <p className="font-semibold">{api.name}</p>
                        <p className="text-xs text-gray-500 truncate">{api.url}</p>
                    </div>
                    <div className="flex gap-2">
                        <button onClick={() => setEditingApi(api)} className="p-2 text-sm text-gray-600 hover:text-blue-600"><i className="fas fa-pen"></i></button>
                        <button onClick={() => handleDelete(api.id)} className="p-2 text-sm text-gray-600 hover:text-red-600"><i className="fas fa-trash"></i></button>
                    </div>
                </div>
            ))}
            <button onClick={() => setEditingApi({ id: null, name: '', format: 'openai', url: '', key: '', model: 'tts-1', voice: 'alloy' })} className="w-full mt-4 py-3 bg-green-500 text-white rounded-md"><i className="fas fa-plus mr-2"></i>添加新语音 API</button>
        </div>
    );
};
...
const SettingsModal = ({ settings, onSave, onClose }) => {
    ...
    useEffect(() => { const fetchSystemVoices = () => { if (!window.speechSynthesis) return; const voices = window.speechSynthesis.getVoices(); if (voices.length > 0) { setSystemVoices(voices.filter(v => v.lang.startsWith('zh') || v.lang.startsWith('en') || v.lang.startsWith('fr') || v.lang.startsWith('es') || v.lang.startsWith('ja') || v.lang.startsWith('ko') || v.lang.startsWith('vi'))); } }; if (window.speechSynthesis) { if (window.speechSynthesis.onvoiceschanged !== undefined) { window.speechSynthesis.onvoiceschanged = fetchSystemVoices; } fetchSystemVoices(); } }, []);

    const handleChange = (key, value) => setTempSettings(prev => ({ ...prev, [key]: value }));
    const handleTtsConfigChange = (key, value) => { setTempSettings(prev => ({ ...prev, thirdPartyTtsConfig: { ...prev.thirdPartyTtsConfig, [key]: value } })); };
    ...
    const handleAddPrompt = () => { const newPrompt = { id: generateSimpleId('prompt'), name: '新助理', content: '你是一个...', openingLine: '你好，我是你的新助理。', model: settings.selectedModel, ttsVoice: 'zh-CN-XiaoxiaoMultilingualNeural', avatarUrl: '' }; const newPrompts = [...(tempSettings.prompts || []), newPrompt]; handleChange('prompts', newPrompts); };
    ...
    const handleAddModel = () => { const newModel = { id: generateSimpleId('model'), name: '新模型', value: '', maxContextTokens: 8192 }; const newModels = [...(tempSettings.chatModels || []), newModel]; handleChange('chatModels', newModels); };
    const handleDeleteModel = (idToDelete) => { if (!window.confirm('确定删除吗？')) return; const newModels = (tempSettings.chatModels || []).filter(m => m.id !== idToDelete); handleChange('chatModels', newModels); };
    const handleModelSettingChange = (modelId, field, value) => { const newModels = (tempSettings.chatModels || []).map(m => m.id === modelId ? { ...m, [field]: value } : m); handleChange('chatModels', newModels); };
    ...
    const handleSaveApis = (newApis) => {
        setTempSettings(prev => ({...prev, customTtsApis: newApis}));
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden relative" style={{ height: 'min(650px, 90vh)' }} onClick={e => e.stopPropagation()}>
                {view === 'main' && ( <div className="p-6 h-full flex flex-col"> <h3 className="text-2xl font-bold mb-4 shrink-0">设置</h3> <div className="space-y-4 flex-grow overflow-y-auto pr-2"> <button type="button" onClick={() => setView('apiKeys')} className="w-full flex justify-between items-center p-3 rounded-md bg-gray-100 hover:bg-gray-200 transition-colors"><h4 className="text-lg font-bold">聊天 API 密钥管理</h4><i className={`fas fa-arrow-right`}></i></button> <button type="button" onClick={() => setView('prompts')} className="w-full flex justify-between items-center p-3 rounded-md bg-gray-100 hover:bg-gray-200 transition-colors"><h4 className="text-lg font-bold">助理工作室</h4><i className={`fas fa-arrow-right`}></i></button> <button type="button" onClick={() => setView('models')} className="w-full flex justify-between items-center p-3 rounded-md bg-gray-100 hover:bg-gray-200 transition-colors"><h4 className="text-lg font-bold">模型管理</h4><i className={`fas fa-arrow-right`}></i></button> <button type="button" onClick={() => setView('knowledgeBases')} className="w-full flex justify-between items-center p-3 rounded-md bg-gray-100 hover:bg-gray-200 transition-colors"><h4 className="text-lg font-bold">知识库管理</h4><i className={`fas fa-arrow-right`}></i></button> 
                
                <div className="p-3 bg-gray-50 rounded-md space-y-4">
                    <h4 className="text-md font-semibold">朗读设置</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div className="flex items-center gap-2"><label className="text-sm shrink-0">语速 ({tempSettings.ttsRate}%)</label><input type="range" min="-100" max="100" step="5" value={tempSettings.ttsRate} onChange={(e) => handleChange('ttsRate', parseInt(e.target.value, 10))} className="w-full"/></div><div className="flex items-center gap-2"><label className="text-sm shrink-0">音调 ({tempSettings.ttsPitch}%)</label><input type="range" min="-100" max="100" step="5" value={tempSettings.ttsPitch} onChange={(e) => handleChange('ttsPitch', parseInt(e.target.value, 10))} className="w-full"/></div></div>
                    <div>
                        <label className="block text-sm font-medium mb-1">朗读引擎</label>
                        <div className="flex gap-2">
                           <select value={tempSettings.activeTtsApiId} onChange={e => handleChange('activeTtsApiId', e.target.value)} className="w-full px-3 py-2 bg-gray-100 border rounded-md">
                               <option value="microsoft">Microsoft Azure</option>
                               {(tempSettings.customTtsApis || []).map(api => ( <option key={api.id} value={api.id}>{api.name}</option> ))}
                           </select>
                           <button onClick={() => setView('ttsApiManager')} className="px-4 py-2 bg-gray-600 text-white rounded-md shrink-0 hover:bg-gray-700">管理</button>
                        </div>
                    </div>
                    
                    {tempSettings.activeTtsApiId === 'microsoft' ? (
                        <div className="space-y-2">
                             <div><label className="block text-sm font-medium mb-1">发音人 (Microsoft)</label><select value={tempSettings.thirdPartyTtsConfig.microsoftVoice} onChange={(e) => handleTtsConfigChange('microsoftVoice', e.target.value)} className="w-full px-3 py-2 bg-gray-100 border rounded-md">{microsoftTtsVoices.map(voice => <option key={voice.value} value={voice.value}>{voice.name}</option>)}</select></div>
                             <div><label className="block text-sm font-medium mb-1">朗读风格</label><select value={tempSettings.ttsStyle} onChange={(e) => handleChange('ttsStyle', e.target.value)} className="w-full px-3 py-2 bg-gray-100 border rounded-md">{SPEECH_STYLES.map(style => <option key={style.value} value={style.value}>{style.name}</option>)}</select></div>
                             <div><label className="block text-sm font-medium mb-1">微软 API 密钥</label><input type="password" value={tempSettings.thirdPartyTtsConfig.apiKey} onChange={(e) => handleTtsConfigChange('apiKey', e.target.value)} placeholder="请输入微软语音服务密钥" className="w-full px-3 py-2 bg-gray-100 border rounded-md" /></div>
                        </div>
                    ) : (
                        <div>
                             <label className="block text-sm font-medium mb-1">语音指令 (可选)</label>
                             <textarea value={tempSettings.ttsInstructions} onChange={(e) => handleChange('ttsInstructions', e.target.value)} className="w-full p-2 bg-gray-100 border rounded-md text-sm" rows="2" placeholder="例如: 请用欢快和兴奋的语气说话"></textarea>
                        </div>
                    )}
                </div>

                {/* 其他设置 UI */}
                <div><label className="block text-sm font-medium mb-1">聊天背景图片</label><div className="flex gap-2"><input type="text" value={tempSettings.chatBackgroundUrl} onChange={(e) => handleChange('chatBackgroundUrl', e.target.value)} placeholder="输入URL或从本地上传" className="w-full px-3 py-2 bg-gray-100 border rounded-md" /><input type="file" ref={fileInputRef} onChange={handleBgImageSelect} accept="image/*" className="hidden" /><button type="button" onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-gray-600 text-white rounded-md shrink-0 hover:bg-gray-700">上传</button></div></div><div className="flex items-center gap-4"><label className="text-sm shrink-0">背景图透明度: {tempSettings.backgroundOpacity}%</label><input type="range" min="0" max="100" step="1" value={tempSettings.backgroundOpacity} onChange={(e) => handleChange('backgroundOpacity', parseInt(e.target.value, 10))} className="w-full"/></div> <div className="flex items-center justify-between"><label className="block text-sm font-medium">始终开启新对话</label><input type="checkbox" checked={tempSettings.startWithNewChat} onChange={(e) => handleChange('startWithNewChat', e.target.checked)} className="h-5 w-5 text-blue-500 rounded" /></div> <div><label className="block text-sm font-medium mb-1">语音识别语言</label><select value={tempSettings.speechLanguage} onChange={(e) => handleChange('speechLanguage', e.target.value)} className="w-full px-3 py-2 bg-gray-100 border rounded-md">{speechLanguageOptions.map(o => <option key={o.value} value={o.value}>{o.name}</option>)}</select></div> <div className="flex items-center justify-between"><label className="block text-sm font-medium">AI 回复后自动朗读</label><input type="checkbox" checked={tempSettings.autoRead} onChange={(e) => handleChange('autoRead', e.target.checked)} className="h-5 w-5 text-blue-500 rounded" /></div> 

                </div> <div className="flex justify-end gap-3 mt-6 shrink-0"><button onClick={onClose} className="px-4 py-2 bg-gray-200 rounded-md">关闭</button><button onClick={() => onSave(tempSettings)} className="px-4 py-2 bg-blue-600 text-white rounded-md">保存</button></div> </div> )}
                {view === 'ttsApiManager' && <SubPageWrapper title="朗读 API 管理" onBack={() => setView('main')}><TtsApiManager customTtsApis={tempSettings.customTtsApis} onSave={handleSaveApis} /></SubPageWrapper>}
                {view === 'apiKeys' && <SubPageWrapper title="聊天 API 密钥管理" onBack={() => setView('main')}><ApiKeyManager apiKeys={tempSettings.apiKeys} activeApiKeyId={tempSettings.activeApiKeyId} onChange={handleApiKeySettingChange} onAdd={handleAddApiKey} onDelete={handleDeleteApiKey} onSetActive={handleSetActiveApiKey} /></SubPageWrapper>}
...
const AiChatAssistant = ({ onClose, initialConfig = {} }) => {
    ...
    useEffect(() => {
        const initializeApp = async () => {
            ...
            const finalSettings = { ...BASE_DEFAULT_SETTINGS, ...initialConfig, ...parsed, thirdPartyTtsConfig: { ...BASE_DEFAULT_SETTINGS.thirdPartyTtsConfig, ...initialConfig.thirdPartyTtsConfig, ...parsed.thirdPartyTtsConfig }, };
            if (initialConfig.apiKeys && initialConfig.apiKeys.length > 0 && (!parsed.apiKeys || parsed.apiKeys.length === 0)) { finalSettings.apiKeys = initialConfig.apiKeys; finalSettings.activeApiKeyId = initialConfig.activeApiKeyId; }
    ...
    useEffect(() => { if (isMounted) { safeLocalStorageSet('ai_assistant_settings_v66_final', JSON.stringify(settings)); } }, [settings, isMounted]);
    ...
    const getKnowledgeBaseContext = (userInput, activeBases) => { if (!activeBases || activeBases.length === 0) return ''; const mockKnowledgeBases = { hsk1: [{ keyword: '你好', content: '你好 (nǐ hǎo) - Hello' }, { keyword: '谢谢', content: '谢谢 (xièxiè) - Thank you' }], my_vocab: [{ keyword: '学习', content: '学习 (xuéxí) - To study; to learn.' }, { keyword: '电脑', content: '电脑 (diànnǎo) - Computer.' }] }; let matchedEntries = []; activeBases.forEach(baseId => { const baseData = mockKnowledgeBases[baseId] || []; baseData.forEach(entry => { if (userInput.includes(entry.keyword)) { matchedEntries.push(entry.content); } }); }); if (matchedEntries.length > 0) { const context = matchedEntries.slice(0, 5).join('\n- '); return `[请优先参考以下本地知识库信息来回答]:\n- ${context}\n\n`; } return ''; };
                ...
                <div className="flex-1 flex flex-col h-full min-w-0 z-10">
                    ...
                    <footer className="flex-shrink-0 p-2 sm:p-4 pb-safe bg-gradient-to-t from-gray-100/80 via-gray-100/50 to-transparent z-10">
                        ...
                        <div className="flex items-center justify-center gap-2 mb-2 max-w-3xl mx-auto">
                           <button onClick={() => createNewConversation()} className="px-3 py-1.5 bg-white rounded-full text-xs text-gray-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),_0_1px_3px_rgba(0,0,0,0.08)] hover:bg-gray-100 transition-all duration-200 active:scale-95" title="新对话"> <i className="fas fa-plus mr-1"></i> <span>新对话</span> </button>
                           <button type="button" onClick={() => setShowModelSelector(true)} className="px-3 py-1.5 bg-white rounded-full text-xs text-gray-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),_0_1px_3px_rgba(0,0,0,0.08)] hover:bg-gray-100 transition-all duration-200 active:scale-95" title="切换模型"> <i className="fas fa-brain mr-1"></i> <span>模型</span> </button>
                           <button type="button" onClick={() => setShowAssistantSelector(true)} className="px-3 py-1.5 bg-white rounded-full text-xs text-gray-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),_0_1px_3px_rgba(0,0,0,0.08)] hover:bg-gray-100 transition-all duration-200 active:scale-95" title="更换助理"> <i className="fas fa-user-tie mr-1"></i> <span>助理</span> </button>
                           <button type="button" onClick={() => setShowKnowledgeBaseSelector(true)} className="px-3 py-1.5 bg-white rounded-full text-xs text-gray-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),_0_1px_3px_rgba(0,0,0,0.08)] hover:bg-gray-100 transition-all duration-200 active:scale-95" title="切换知识库"> <i className="fas fa-book mr-1"></i> <span>知识库</span> </button>
                        </div>
                        <form onSubmit={(e)=>{e.preventDefault();handleSubmit(false)}} className="flex items-end w-full max-w-3xl mx-auto p-2 bg-white backdrop-blur-sm rounded-2xl border border-gray-200/80 transition-shadow duration-300 ease-in-out hover:shadow-2xl focus-within:shadow-2xl shadow-[0_-10px_25px_rgba(0,0,0,0.05),_0_-5px_10px_rgba(0,0,0,0.04)]">
                            ...
                            <div className="flex items-center flex-shrink-0 ml-1">
                                {!showSendButton ? ( <button type="button" onClick={isListening ? stopListening : startListening} className={`w-10 h-10 flex items-center justify-center rounded-full transition-colors ${isListening ? 'text-white bg-red-500 animate-pulse' : 'text-gray-500 hover:bg-gray-500/10'}`} title="语音输入"> <i className="fas fa-microphone text-xl"></i> </button> ) : ( <button type="submit" className="w-10 h-10 flex items-center justify-center bg-blue-600 text-white rounded-full shadow-lg shadow-blue-500/30 hover:bg-blue-700 disabled:opacity-50 transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95" disabled={isLoading}> <i className="fas fa-arrow-up text-xl"></i> </button> )}
                            </div>
                        </form>
                    </footer>
                </div>
