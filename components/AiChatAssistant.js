// /components/AiChatAssistant.js - v39 (最终整合版 - 全屏、激活、所有UX优化及Bug修复)
import React, { useState, useEffect, useRef, useCallback, useMemo } from 'react';
import AiTtsButton from './AiTtsButton';
import FingerprintJS from '@fingerprintjs/fingerprintjs'; // 用于生成设备指纹
...
const ChatSidebar = ({ isOpen, conversations, currentId, onSelect, onNew, onDelete, onRename, prompts, settings }) => {
    ...
    const renderConversationItem = (conv) => (
        <div key={conv.id} className={`group flex items-center p-2 rounded-md cursor-pointer ${currentId === conv.id ? 'bg-primary/20' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`} onClick={(e) => { e.stopPropagation(); onSelect(conv.id); setActiveMenu(null); }}>
            ...
            <div className="flex-grow overflow-y-auto space-y-2">
                ...
                {groupedConversations.uncategorized.length > 0 && (
                     <details className="group" open>
                        ...
                        <div className="pl-3 mt-1 space-y-1"> {groupedConversations.uncategorized.map(renderConversationItem)} </div>
                    </details>
                )}
            </div>
        </div>
            ...
            {prompts.map(p => (
                <div key={p.id} className="p-3 bg-gray-50 dark:bg-gray-900/50 rounded-md border dark:border-gray-700">
                    ...
                    <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                        <div><label className="text-xs font-medium">模型:</label><select value={p.model || settings.selectedModel} onChange={(e) => onChange(p.id, 'model', e.target.value)} className="w-full mt-1 px-2 py-1 bg-white dark:bg-gray-800 border rounded-md text-xs">{CHAT_MODELS.map(m => <option key={m.value} value={m.value}>{m.name}</option>)}</select></div>
...
const SettingsModal = ({ settings, onSave, onClose }) => {
    ...
    const handleAddPrompt = () => { const newPrompt = { id: `custom-${Date.now()}`, name: '新提示词', content: '请输入...', model: settings.selectedModel, ttsVoice: 'zh-CN-XiaoxiaoMultilingualNeural', avatarUrl: '' }; const newPrompts = [...tempSettings.prompts, newPrompt]; handleChange('prompts', newPrompts); };
    ...
    // 定义 microsoftTtsVoices 列表，确保在 SettingsModal 和 PromptManager 中都可用
                ...
                {view === 'main' && (
                    <div className="p-6 h-full flex flex-col">
                        ...
                        <div className="space-y-4 flex-grow overflow-y-auto pr-2">
                            ...
                            <div><button type="button" onClick={() => setView('prompts')} className="w-full flex justify-between items-center p-3 rounded-md bg-gray-100 dark:bg-gray-700/50 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"><h4 className="text-lg font-bold">提示词工作室</h4><i className={`fas fa-arrow-right`}></i></button></div>
                            <div><label className="block text-sm font-medium mb-1">聊天背景图片 URL</label><input type="text" value={tempSettings.chatBackgroundUrl} onChange={(e) => handleChange('chatBackgroundUrl', e.target.value)} placeholder="https://..." className="w-full px-3 py-2 bg-gray-100 dark:bg-gray-700 border rounded-md" /></div>
                            <div className="flex items-center justify-between"><label className="block text-sm font-medium">始终开启新对话</label><input type="checkbox" checked={tempSettings.startWithNewChat} onChange={(e) => handleChange('startWithNewChat', e.target.checked)} className="h-5 w-5 text-primary rounded" /></div>
                            <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md space-y-3"><label className="block text-sm font-medium">高级参数</label><div className="flex items-center gap-4"><label className="text-sm shrink-0">温度: {tempSettings.temperature}</label><input type="range" min="0" max="1" step="0.1" value={tempSettings.temperature} onChange={(e) => handleChange('temperature', parseFloat(e.target.value))} className="w-full"/></div><div><div className="flex items-center justify-between"><label htmlFor="thinking-mode-toggle" className="block text-sm font-medium">关闭 2.5 系列模型思考模式</label><input id="thinking-mode-toggle" type="checkbox" checked={tempSettings.disableThinkingMode} onChange={(e) => handleChange('disableThinkingMode', e.target.checked)} className="h-5 w-5 text-primary rounded cursor-pointer" /></div><p className="text-xs text-gray-500 mt-1">开启后可大幅提升响应速度和降低成本，但可能影响复杂问题的回答质量。</p></div></div>
                        ...
                        <div className="flex justify-end gap-3 mt-6 shrink-0"><button onClick={onClose} className="px-4 py-2 bg-gray-200 dark:bg-gray-600 rounded-md">关闭</button><button onClick={() => onSave(tempSettings)} className="px-4 py-2 bg-primary text-white rounded-md">保存</button></div>
                    </div>
                )}
                {view === 'prompts' && (<PromptManager prompts={tempSettings.prompts} settings={tempSettings} onBack={() => setView('main')} onChange={handlePromptSettingChange} onAdd={handleAddPrompt} onDelete={handleDeletePrompt} microsoftTtsVoices={microsoftTtsVoices} />)}
            </div>
        </div>
    );
};
...
const DEFAULT_SETTINGS = { apiKey: '', selectedModel: 'gemini-2.5-flash', chatModels: [ { name: 'Gemini 2.5 Flash', value: 'gemini-2.5-flash' }, { name: 'Gemini 2.5 Pro', value: 'gemini-2.5-pro' }, { name: 'Gemini 2.0 Flash', value: 'gemini-2.0-flash' }, { name: 'Gemini 1.5 Flash (最新)', value: 'gemini-1.5-flash-latest' }, { name: 'Gemini 1.5 Pro (最新)', value: 'gemini-1.5-pro-latest' }, ], temperature: 0.8, maxOutputTokens: 2048, disableThinkingMode: true, startWithNewChat: false, prompts: DEFAULT_PROMPTS, currentPromptId: DEFAULT_PROMPTS[0]?.id || '', autoRead: false, ttsEngine: TTS_ENGINE.THIRD_PARTY, thirdPartyTtsVoice: 'zh-CN-XiaoxiaoMultilingualNeural', systemTtsVoiceURI: '', speechLanguage: 'zh-CN', chatBackgroundUrl: '/images/chat-bg.jpg', userAvatarUrl: '/images/user-avatar.png', aiAvatarUrl: '/images/ai-avatar.png', };

const AssistantSelector = ({ prompts, settings, onSelect, onClose }) => {
    // 确保 microsoftTtsVoices 在这里可用，否则需要作为 prop 传递
    const microsoftTtsVoices = [
        { name: '晓晓 (女, 多语言)', value: 'zh-CN-XiaoxiaoMultilingualNeural' }, { name: '晓辰 (女, 多语言)', value: 'zh-CN-XiaochenMultilingualNeural' }, { name: '云希 (男, 温和)', value: 'zh-CN-YunxiNeural' }, { name: '云泽 (男, 叙事)', value: 'zh-CN-YunzeNeural' }, { name: '晓晓 (女, 亲切)', value: 'zh-CN-XiaoxiaoNeural' }, { name: '晓颜 (女)', value: 'zh-CN-XiaoyanNeural'}, { name: '晓伊 (女, 动漫)', value: 'zh-CN-XiaoyiNeural' }, { name: '云健 (男, 沉稳)', value: 'zh-CN-YunjianNeural' }, { name: '云扬 (男, 阳光)', value: 'zh-CN-YunyangNeural' }, { name: '晓臻 (女, 台湾)', value: 'zh-TW-HsiaoChenNeural' }, { name: '允喆 (男, 台湾)', value: 'zh-TW-YunJheNeural' }, { name: 'Ava (女, 美国, 多语言)', value: 'en-US-AvaMultilingualNeural' }, { name: 'Steffan (男, 美国, 多语言)', value: 'en-US-SteffanMultilingualNeural' }, { name: 'Vivienne (女, 法国, 多语言)', value: 'fr-FR-VivienneMultilingualNeural' }, { name: 'Remy (男, 法国, 多语言)', value: 'fr-FR-RemyMultilingualNeural' }, { name: '妮拉 (女, 缅甸)', value: 'my-MM-NilarNeural' }, { name: '蒂哈 (男, 缅甸)', value: 'my-MM-ThihaNeural' }, { name: '怀眉 (女, 越南)', value: 'vi-VN-HoaiMyNeural' }, { name: '南明 (男, 越南)', value: 'vi-VN-NamMinhNeural' },
    ];

    return (
    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm z-20 flex flex-col p-4 animate-fade-in" onClick={onClose}>
        <div className="w-full max-w-2xl m-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b dark:border-gray-700 text-center relative"><h3 className="text-lg font-bold">更换助理</h3><button onClick={onClose} className="absolute top-2 right-2 p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"><i className="fas fa-times"></i></button></div>
            <div className="p-4 grid grid-cols-2 sm:grid-cols-3 gap-4 overflow-y-auto max-h-[60vh]">
                {prompts.map(p => (
                    <button key={p.id} onClick={() => onSelect(p.id)} className={`flex flex-col items-center p-4 rounded-lg border-2 transition-all ${settings.currentPromptId === p.id ? 'border-primary bg-primary/10' : 'border-transparent hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                        <img src={convertGitHubUrl(p.avatarUrl) || convertGitHubUrl(settings.aiAvatarUrl)} alt={p.name} className="w-16 h-16 rounded-full object-cover mb-2 shadow-md"/>
...
const AiChatAssistant = ({ onClose, isFullScreenMode = false }) => {
    // --- 激活码系统状态 ---
    const [activationState, setActivationState] = useState('checking'); // 'checking', 'unactivated', 'activated', 'error'
    const [activationKey, setActivationKey] = useState('');
    const [activationError, setActivationError] = useState('');
    const [isActivating, setIsActivating] = useState(false);
    const [keyType, setKeyType] = useState(null); // 'trial' or 'permanent'
    const [trialExpiryInfo, setTrialExpiryInfo] = useState(''); // 用于显示试用期截止日期或剩余天数
    // ----------------------------
    ...
    // isFullScreen is now controlled by parent Layout for global full screen
    ...
    useEffect(() => {
        ...
        const initializeApp = async () => {
            ...
            if (savedSettings) {
                ...
                // 确保 prompts 和 chatModels 都有默认值或已保存的值
                ...
                if (!parsed.chatModels || parsed.chatModels.length === 0) {
                    parsed.chatModels = DEFAULT_CHAT_MODELS_LIST; // 使用默认模型列表
                } else {
                    // 确保旧的模型列表兼容新结构
                    parsed.chatModels = parsed.chatModels.map(m => ({
                        name: m.name || m.value, // 兼容旧版本可能只有value
                        value: m.value
                    }));
                }
            ...
            // 激活验证流程
            setActivationState('checking'); // 开始检查激活状态
            try {
                ...
                if (storedKey) {
                    ...
                    if (response.ok && data.success) {
                        ...
                        if (data.keyType === 'trial') {
                            const expiry = new Date(data.activatedAt + (data.durationDays || 7) * 24 * 60 * 60 * 1000);
                            setTrialExpiryInfo(`试用中，有效期至: ${expiry.toLocaleDateString()} ${expiry.toLocaleTimeString()}`);
                        } else {
                        ...
                        setActivationError(data.message || '本地激活码已失效，请重新激活或试用。');
                    }
                }
                ...
                if (!activated) {
                    ...
                    if (trialResponse.ok && trialData.success) {
                        ...
                        const expiry = new Date(Date.now() + (trialData.durationDays || 7) * 24 * 60 * 60 * 1000);
                        setTrialExpiryInfo(`试用中，有效期至: ${expiry.toLocaleDateString()} ${expiry.toLocaleTimeString()}`);
                        activated = true;
                    } else {
                        setActivationState('unactivated');
                        setActivationError(trialData.message || '无法自动开始试用，请手动输入激活码。');
                    }
                }

            } catch (error) {
                console.error("Activation initialization error:", error);
                setActivationState('unactivated');
                setActivationError('网络错误，无法连接到激活服务器。请检查网络。');
            }
    ...
    useEffect(() => {
        if (!currentConversation || !settings.autoRead || !isMounted || activationState !== 'activated') return; // 只有激活状态才自动朗读
        ...
        if (currentMessageCount > lastKnownCount && lastMessage?.role === 'ai' && lastMessage?.content && lastMessage?.timestamp && (Date.now() - lastMessage.timestamp < 2000)) {
            setTimeout(() => {
    ...
    }, [currentConversation, settings.autoRead, isMounted, activationState]); // 增加 activationState 依赖
    ...
    const handleActivate = async (e) => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                if (data.keyType === 'trial') { const expiry = new Date(data.activatedAt + (data.durationDays || 7) * 24 * 60 * 60 * 1000); setTrialExpiryInfo(`试用中，有效期至: ${expiry.toLocaleDateString()} ${expiry.toLocaleTimeString()}`); } else { setTrialExpiryInfo('永久授权'); }
            } else { throw new Error(data.message || '激活失败，请检查激活码或联系管理员。'); }
    ...
    const handleSubmit = async (isRegenerate = false) => {
        if (!currentConversation || isLoading || activationState !== 'activated') return; // 只有激活状态才能发送消息
        ...
        try {
            ...
            const history = messagesForApi.map(msg => { const parts = []; if (msg.content) parts.push({ text: msg.content }); if (msg.images) msg.images.forEach(img => parts.push({ inlineData: { mimeType: img.type, data: img.data } })); return { role: msg.role === 'user' ? 'user' : 'model', parts }; });
                ...
                <div className="w-full max-w-sm bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 text-center">
                    ...
                    <form onSubmit={handleActivate} className="space-y-4">
                        <input
                            ...
                            placeholder="在此输入永久激活码"
                            className="w-full px-4 py-2 text-center bg-gray-100 dark:bg-gray-700 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
                        />
                        {activationError && <p className="text-red-500 text-sm">{activationError}</p>}
                        <button
                            type="submit"
                            disabled={isActivating}
                            className="w-full px-4 py-2 bg-primary text-white rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                        >
    ...
    return (
        <div className={containerClasses} style={containerStyle}>
            {/* ChatSidebar 组件需要传递 prompts 和 settings 才能正确渲染分组 */}
            ...
            <div className="flex-1 flex flex-col h-full min-w-0 relative" data-conv-id={currentConversation?.id}>
                <div className="flex items-center justify-between py-1 px-2 border-b dark:border-gray-700 shrink-0">
                    ...
                    <div className="text-center flex-grow">
                        ...
                        {trialExpiryInfo && <div className="text-xs text-green-500 font-semibold">{trialExpiryInfo}</div>}
                    </div>
