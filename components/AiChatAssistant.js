// /components/AiChatAssistant.js - v49 (最终完整版 - 修复所有已知Bug，采用“报名课程”激活界面设计)
...
const AiChatAssistant = ({ onClose, isFullScreenMode = false }) => {
    ...
    if (activationState !== 'activated') {
        return (
            <div className={containerClasses} style={containerStyle}>
                <div className="w-full h-full flex flex-col items-center justify-center bg-gray-100 dark:bg-gray-900 p-4 animate-fade-in-long">
                    <div className="w-full max-w-sm bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 flex flex-col items-center">
                        <div className="mb-6 w-full animate-slide-in-up delay-100">
                            ...
                            <h2 className="text-2xl font-bold mb-2 text-center text-primary">报名中文课程</h2>
                            <p className="text-gray-600 dark:text-gray-400 text-sm mb-2 text-center">【课程介绍】结合中缅教学方案，高效学习中文，价格比大部分缅甸机构更优惠！</p>
                            <p className="text-gray-600 dark:text-gray-400 text-sm mb-2 text-center">【地址】仰光某区，欢迎线下咨询！</p>
                            <p className="text-xl font-bold text-green-500 mb-4 text-center">【优惠价格】AI助手套餐：$50 / 月（原价$80）</p>
                            <p className="text-gray-600 dark:text-gray-400 text-sm mb-4 text-center">请通过以下方式联系我们，获取专属学习方案：</p>
                        </div>
                        ...
                        <form onSubmit={handleActivate} className="space-y-4 w-full">
                            <input type="text" value={activationKey} onChange={(e) => setActivationKey(e.target.value)} placeholder="在此输入您的激活码" className="w-full px-4 py-3 text-center bg-gray-100 dark:bg-gray-700 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary animate-slide-in-up delay-600"/>
                            <button type="submit" disabled={isActivating} className="w-full py-3 px-4 bg-primary text-white rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transition-all duration-300 transform hover:-translate-y-1 shadow-lg animate-slide-in-up delay-700">
                                {isActivating ? (<><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2"></div> 正在验证...</>) : "立即激活"}
                            </button>
                        </form>
                    </div>
                    {isFullScreenMode && <button onClick={onClose} className="mt-4 px-6 py-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 bg-white/50 dark:bg-gray-700/50 rounded-full shadow-md transition-all duration-300 hover:scale-105 animate-fade-in delay-800">返回网站</button>}
                </div>
            </div>
        );
    }
                ...
                <div className="p-3 border-t dark:border-gray-700 shrink-0">
                    ...
                    {isLoading ? ( <div className="flex justify-center items-center gap-2 text-gray-500"><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-primary"></div> 正在思考中...</div> ) : (
                        <form onSubmit={(e)=>{e.preventDefault();handleSubmit(false)}} className="flex items-end gap-2">
                            ...
                            <button type="button" onClick={() => console.log('This button is for non-fullscreen mode')} className="p-3 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 shrink-0" title={'全屏模式'}> <i className={`fas fa-expand`}></i> </button>
                        </form>
                    )}
                </div>
            </div>
