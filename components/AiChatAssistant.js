// /components/AiChatAssistant.js - v57 (功能增强版 - 满足所有新需求)
import React, { useState, useEffect, useRef, useCallback, useMemo } from 'react';
import AiTtsButton from './AiTtsButton'; // 假设 AiTtsButton 也有相应更新
...
const CHAT_MODELS_LIST = [
    ...
    { name: 'Gemini 2.0 Flash', value: 'gemini-2.0-flash', maxContext: 8192 },
...
const AiChatAssistant = ({ onClose }) => {
    ...
    useEffect(() => {
        const initializeApp = async () => {
            setIsMounted(true);
            let finalSettings = JSON.parse(JSON.stringify(DEFAULT_SETTINGS)); // Deep copy
            const savedSettings = safeLocalStorageGet('ai_assistant_settings_v23_plus');
            if (savedSettings) {
                ...
                finalSettings = {
                    ...finalSettings,
                    ...parsed,
                    apiKeys: parsed.apiKeys && parsed.apiKeys.length > 0 ? parsed.apiKeys : finalSettings.apiKeys,
                    chatModels: parsed.chatModels && parsed.chatModels.length > 0 ? parsed.chatModels : finalSettings.chatModels,
                    prompts: (parsed.prompts || []).map(p => ({
                        ...p,
                        isDefault: DEFAULT_PROMPTS.some(dp => dp.id === p.id), // 重新标记默认
                    })),
                };
            }
            ...
            const savedConversations = safeLocalStorageGet('ai_assistant_conversations_v23_plus');
    ...
    useEffect(() => { if (isMounted) { safeLocalStorageSet('ai_assistant_settings_v23_plus', JSON.stringify(settings)); safeLocalStorageSet('ai_assistant_conversations_v23_plus', JSON.stringify(conversations)); } }, [settings, conversations, isMounted]);
    ...
    useEffect(() => { if (!currentConversation || !settings.autoRead || !isMounted) return; const lastMessage = currentConversation.messages[currentConversation.messages.length - 1]; if (lastMessage && lastMessage.role === 'ai' && lastMessage.content && !lastMessage.isTyping && lastMessage.timestamp > (lastAutoReadMessageId.current || 0)) { lastAutoReadMessageId.current = lastMessage.timestamp; setTimeout(() => { const bubble = document.getElementById(`msg-${currentConversation.id}-${currentConversation.messages.length - 1}`); if (bubble && document.body.contains(bubble)) { bubble.querySelector('button[title="朗读"]')?.click(); } }, 300); } }, [currentConversation?.messages, settings.autoRead, isMounted, currentConversation?.id]);
    ...
    const handleSubmit = async (isRegenerate = false) => {
        ...
        if (!activeApiKey || !activeApiKey.key) {
            setError('请在设置中配置并选择一个有效的 API 密钥。');
            return;
        }
        ...
        try {
            ...
            const modelInfo = (settings.chatModels || []).find(m => m.value === (currentPrompt.model || settings.selectedModel)) || settings.chatModels[0];
            ...
            if (activeApiKey.type === API_TYPES.OPENAI) {
                ...
                if (!settings.openaiCompatibleEndpoint) {
                    throw new Error("请在设置中配置 OpenAI 兼容接口地址。");
                }
                const messages = contextMessages.map(msg => {
                    const content = [];
                    if (msg.content) content.push({ type: "text", text: msg.content });
                    if (msg.images) msg.images.forEach(img => content.push({ type: "image_url", image_url: { url: `data:${img.type};base64,${img.data}` }}));
                    return { role: msg.role === 'ai' ? 'assistant' : 'user', content };
                });
                const systemPrompt = { role: 'system', content: currentPrompt.content };
                const body = {
                    model: modelToUse,
                ...
                response = await fetch(settings.openaiCompatibleEndpoint + '/chat/completions', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${activeApiKey.key}` },
                ...
                response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${modelToUse}:generateContent?key=${activeApiKey.key}`, {
                    method: 'POST',
            ...
            <div className="relative z-10 flex flex-1 min-h-0">
                ...
                <div className="flex-1 flex flex-col h-full min-w-0">
                    <div className="flex items-center justify-between py-1 px-2 border-b border-white/20 dark:border-gray-700/50 shrink-0 bg-white/30 dark:bg-black/30 backdrop-blur-md">
                        ...
                        <div className="text-center flex-grow"> <h2 className="text-lg font-semibold truncate">{currentConversation?.title || '聊天'}</h2> <div className={`text-xs font-semibold text-green-500`}>在线</div> </div>
