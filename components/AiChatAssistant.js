// /components/AiChatAssistant.js - v24: 语音识别后3秒自动发送
...
const AiChatAssistant = () => {
    ...
    const autoSendTimeoutRef = useRef(null); // 新增：用于自动发送的定时器
    ...
    const startListening = useCallback(() => {
        ...
        recognition.maxAlternatives = 1; // 只需要一个最佳结果
        ...
        recognition.onresult = (e) => {
            ...
            setUserInput(transcript); // 填充输入框

            // 修复：语音识别完成后自动发送 (3秒延时)
            // 只有当没有选择图片时才自动发送
            if (selectedImages.length === 0 && transcript) {
                if (autoSendTimeoutRef.current) clearTimeout(autoSendTimeoutRef.current); // 清除旧的定时器
                autoSendTimeoutRef.current = setTimeout(() => {
                    // 再次检查用户输入和图片，确保没有在延时期间被修改
                    if (transcript === userInput && selectedImages.length === 0) {
                         handleSubmit(false, transcript);
                    }
                    autoSendTimeoutRef.current = null;
                }, 3000); // 3秒后自动发送
            }
        ...
        recognition.onend = () => {
            setIsListening(false);
            // 如果自动发送定时器还在运行，说明还没有发送，就不需要在这里额外处理了。
            // 如果定时器已经为null（因为发送了或者被取消了），也不需要额外处理。
        };
        recognition.start();
        recognitionRef.current = recognition;
    }, [settings.speechLanguage, selectedImages, userInput]); // 依赖 userInput 确保 autoSendTimeoutRef 比较正确性
    ...
    const handleSubmit = async (isRegenerate = false, voiceText = '') => {
        // 如果有自动发送定时器在运行，先取消
        if (autoSendTimeoutRef.current) {
            clearTimeout(autoSendTimeoutRef.current);
            autoSendTimeoutRef.current = null;
        ...
        const textToProcess = voiceText || userInput.trim();
