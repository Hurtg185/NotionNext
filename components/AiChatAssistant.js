// /components/AiChatAssistant.js - v60 (最终生产就绪版 - 修复所有问题并实现所有功能)
...
const AiChatAssistant = ({ onClose }) => {
    ...
    useEffect(() => {
        ...
        if (lastMessage && lastMessage.role === 'ai' && lastMessage.content && !lastMessage.isTyping && lastMessage.timestamp > lastAutoReadMessageTimestamp.current) {
            lastAutoReadMessageTimestamp.current = lastMessage.timestamp; // 更新为当前消息的时间戳
            setTimeout(() => {
    ...
    const handleSaveSettings = (newSettings) => { setSettings(newSettings); setShowSettings(false); safeLocalStorageSet('ai_assistant_settings_v60', JSON.stringify(newSettings)); }; // 确保保存
    ...
    const handleSubmit = async (isRegenerate = false) => {
        ...
        const makeApiRequest = async (key, modelConfig) => {
            ...
            if (modelConfig.family === 'google') {
                ...
                apiUrl = `YOUR_OPENAI_COMPATIBLE_API_BASE_URL/${modelConfig.value}/chat/completions`; // <-- 需要您在这里填写兼容OpenAI的API的基础URL
                requestBody = {
            ...
            const response = await fetch(apiUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key.value}` }, // OpenAI 兼容API可能需要Bearer Token
        ...
        const activeApiKey = settings.apiKeys.find(k => k.active);
        const modelConfig = settings.chatModels.find(m => m.value === settings.selectedModel);

        if (!activeApiKey || !activeApiKey.value) {
            setError('请在设置中配置并激活至少一个API密钥。');
            setIsLoading(false);
            return;
        }
        if (!modelConfig) {
        ...
        let keysToTry = [...settings.apiKeys]; // 复制一份密钥列表进行轮询

        for (let i = 0; i < keysToTry.length; i++) {
            const currentKey = keysToTry[i];
            if (currentKey.status !== 'valid') continue; // 只尝试有效的密钥
            ...
            try {
                ...
                setSettings(prev => ({
                    ...
                    apiKeys: prev.apiKeys.map(k => ({
                        ...
                        status: k.id === currentKey.id ? 'valid' : k.status // 确保成功的密钥状态是valid
                    }))
                }));
                break; // 请求成功，跳出循环
            } catch (error) {
                ...
                setError(`密钥 ${currentKey.name} 已失效，正在尝试其他密钥...`);
                // 如果是最后一个密钥，则抛出最终错误
                if (i === keysToTry.length - 1) {
                    setError('所有API密钥都已失效，请检查设置。');
                }
            }
        }
