// /components/AiChatAssistant.js - v65.9 (Complete Single-File Refactor)
...
const DEFAULT_SETTINGS = {
    ...
    autoRead: false, ttsEngine: TTS_ENGINE.THIRD_PARTY,
    thirdPartyTtsConfig: { provider: 'microsoft', microsoftVoice: 'zh-CN-XiaoxiaoMultilingualNeural', apiUrl: '', apiKey: '', model: 'tts-1', voice: 'alloy', rate: 0, pitch: 0, },
    systemTtsVoiceURI: '', speechLanguage: 'zh-CN',
    chatBackgroundUrl: '/images/chat-bg-light.jpg', chatBackgroundOpacity: 70, // 新增背景透明度设置
    userAvatarUrl: '/images/user-avatar.png', aiAvatarUrl: '/images/ai-avatar.png', isFacebookApp: false,
    activeKnowledgeBases: ['user_vocab'], // 默认启用用户生词本
};

// --- 子组件 (全部内联到 AiChatAssistant.js 中) ---

const AiTtsButton = ({ text, ttsSettings, isSSML = false }) => {
    ...
    const speak = useCallback(async () => {
        ...
        if (utteranceRef.current && window.speechSynthesis) window.speechSynthesis.cancel();

        const { ttsEngine, thirdPartyTtsConfig, systemTtsVoiceURI, speechLanguage } = ttsSettings;

        if (ttsEngine === TTS_ENGINE.SYSTEM) {
            const utterance = new SpeechSynthesisUtterance(text);
            utterance.lang = speechLanguage;
            utterance.rate = 1 + (thirdPartyTtsConfig.rate / 100);
            utterance.pitch = 1 + (thirdPartyTtsConfig.pitch / 100);
            if (systemTtsVoiceURI) {
                const voices = window.speechSynthesis.getVoices();
                utterance.voice = voices.find(v => v.voiceURI === systemTtsVoiceURI);
            }
            utteranceRef.current = utterance;
            window.speechSynthesis.speak(utterance);
        } else if (ttsEngine === TTS_ENGINE.THIRD_PARTY) {
            const { provider, microsoftVoice, apiUrl, apiKey, model, voice, rate, pitch } = thirdPartyTtsConfig;
            try {
                let requestUrl = '';
                let requestHeaders = {};
                ...
                if (provider === 'microsoft') {
                    if (apiUrl && (apiUrl.includes('zuoban') || apiUrl.includes('leftsite'))) {
                        const cleanedText = text.replace(/<[^>]+>/g, '');
                        const encodedText = encodeURIComponent(cleanedText);
                        requestUrl = `${apiUrl}?t=${encodedText}&v=${microsoftVoice}&r=${rate}&p=${pitch}&o=audio-24khz-48kbitrate-mono-mp3`;
                        method = 'GET';
                    } else {
                        requestUrl = apiUrl || 'https://eastasia.tts.speech.microsoft.com/cognitiveservices/v1';
                        requestHeaders = { 'Content-Type': 'application/ssml+xml', 'X-Microsoft-OutputFormat': 'audio-24khz-48kbitrate-mono-mp3', 'Ocp-Apim-Subscription-Key': apiKey };
                        requestBody = text; // text is already SSML
                    }
                } else if (provider === 'openai') {
                    requestUrl = `${apiUrl || 'https://api.openai.com/v1'}/audio/speech`;
                    requestHeaders = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` };
                    const openaiSpeed = Math.max(0.25, Math.min(4.0, 1 + (rate / 100)));
                    requestBody = JSON.stringify({ model: model || 'tts-1', input: text, voice: voice || 'alloy', speed: openaiSpeed });
                } else { throw new Error(`未知的 TTS 提供商: ${provider}`); }

                const response = await fetch(requestUrl, { method, headers: requestHeaders, body: requestBody });
    ...
    return (
        <button onClick={speak} className="p-2 rounded-full hover:bg-black/10" title="朗读">
...
const AiTtsButtonModified = ({ text, ttsSettings }) => {
    ...
    const modifiedText = cleanTextForSpeech(text);
    const { provider, microsoftVoice, rate, pitch, apiUrl } = ttsSettings.thirdPartyTtsConfig;
    let isSSML = false;
    let finalText = modifiedText;

    if (provider === 'microsoft' && !(apiUrl || '').includes('zuoban')) {
        isSSML = true;
        const lang = microsoftVoice.split('-')[0] || 'zh-CN';
        finalText = `<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="${lang}"><voice name="${microsoftVoice}"><prosody rate="${rate}%" pitch="${pitch}%">${modifiedText}</prosody></voice></speak>`;
    }

    return <AiTtsButton text={finalText} ttsSettings={ttsSettings} isSSML={isSSML} />;
};
            ...
            <div className={`p-3 rounded-2xl text-left flex flex-col transition-shadow duration-300 ${isUser ? userBubbleClass : aiBubbleClass}`} style={{ maxWidth: '85%' }}>
                ...
                {!isUser && msg.content && !msg.isTyping && (
                    <div className="flex items-center gap-2 mt-2 -mb-1 text-gray-500">
                        ...
                        {!settings.isFacebookApp && <AiTtsButtonModified text={msg.content} ttsSettings={settings} />}
...
const AiChatAssistant = ({ onClose }) => {
    ...
    // 知识库数据加载（暂时为空，待您提供数据源）
    const allKnowledgeData = useMemo(() => ({
        hsk1: [], hsk2: [], hsk3: [], hsk4: [], hsk5: [], hsk6: [], user_vocab: [], user_notes: [],
    }), []);
                ...
                <div className="flex-1 flex flex-col h-full min-w-0 z-10">
                    ...
                    <footer className="flex-shrink-0 px-4 pt-2 pb-safe bg-gradient-to-t from-gray-100 via-gray-100/80 to-transparent z-10">
                        ...
                        <div className="flex items-center justify-center gap-2 mb-2 max-w-2xl mx-auto">
                             <button onClick={() => createNewConversation()} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/60 backdrop-blur-sm border border-gray-300/80 rounded-full text-gray-700 text-xs hover:bg-gray-100 shadow-sm" title="新对话"> <i className="fas fa-plus"></i> </button>
                            <button type="button" onClick={() => setShowModelSelector(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/60 backdrop-blur-sm border border-gray-300/80 rounded-full text-gray-700 text-xs hover:bg-gray-100 shadow-sm" title="切换模型"> <i className="fas fa-brain"></i> </button>
                            <button type="button" onClick={() => setShowAssistantSelector(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/60 backdrop-blur-sm border border-gray-300/80 rounded-full text-gray-700 text-xs hover:bg-gray-100 shadow-sm" title="更换助理"> <i className="fas fa-user-tie"></i> </button>
                             <button type="button" onClick={() => setShowKnowledgeSelector(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/60 backdrop-blur-sm border border-gray-300/80 rounded-full text-gray-700 text-xs hover:bg-gray-100 shadow-sm" title="切换知识库"> <i className="fas fa-book"></i> </button>
                        </div>
