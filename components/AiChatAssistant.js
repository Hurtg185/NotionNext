// /components/AiChatAssistant.js - v59 (恢复激活码 + 亮色模式 + 滚动优化 + 立体UI优化版)
...
const MessageBubble = ({ msg, settings, isLastAiMessage, onRegenerate, onTypingComplete, onTypingUpdate }) => { 
    ...
    // [代码修改] 应用新的气泡样式，实现立体悬浮效果
    const userBubbleClass = 'bg-primary text-white rounded-xl rounded-br-md shadow-[0_2px_8px_rgba(74,144,226,0.25),_0_8px_20px_rgba(74,144,226,0.15)]'; 
    const aiBubbleClass = 'bg-white text-gray-800 rounded-xl rounded-bl-md shadow-[0_2px_5px_rgba(0,0,0,0.08),_0_8px_20px_rgba(0,0,0,0.04),inset_0_1px_0_rgba(255,255,255,0.7)]'; 
    return ( 
        <div className={`flex items-end gap-2.5 my-4 ${isUser ? 'justify-end' : 'justify-start'}`}> 
            ...
            {/* [代码修改] 移除此处的 rounded-2xl，因为样式已移至上面的 specific class 中 */}
            <div className={`p-3 text-left flex flex-col ${isUser ? userBubbleClass : aiBubbleClass}`} style={{ maxWidth: '85%' }}> 
                ...
                {!isUser && msg.content && !msg.isTyping && ( 
                    <div className="flex items-center gap-2 mt-2 -mb-1 text-gray-500"> 
                        ...
                        {isLastAiMessage && ( 
                            <button onClick={(e) => { e.stopPropagation(); onRegenerate(); }} className="p-2 rounded-full hover:bg-black/10" title="重新生成"><i className="fas fa-sync-alt"></i></button> 
                        )} 
                    </div> 
                )} 
            </div> 
        ...
        <div {...swipeHandlers} className="w-full h-full flex flex-col bg-cover bg-center text-gray-800" style={{ backgroundImage: `url('${convertGitHubUrl(settings.chatBackgroundUrl)}')`}}>
            ...
            <div className="relative z-10 flex flex-1 min-h-0">
                ...
                <div className="flex-1 flex flex-col h-full min-w-0">
                    {/* [代码修改] 应用新的顶部栏样式 */}
                    <header className="flex items-center justify-between py-1 px-2 border-b border-gray-200/30 shrink-0 bg-white/85 backdrop-blur-md shadow-sm">
                    ...
                    {/* [代码修改] 优化底部渐变背景 */}
                    <footer className="flex-shrink-0 px-4 pt-2 pb-safe bg-gradient-to-t from-white/80 via-white/60 to-transparent z-10">
                                ...
                                <div className="flex items-center gap-2 overflow-x-auto p-1 bg-gray-100/50 rounded-lg">
                                    {selectedImages.map((img, index) => (
                                        <div key={index} className="relative shrink-0">
                                            ...
                                            <button onClick={() => removeSelectedImage(index)} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs shadow-md">×</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        {/* [代码修改] 应用新的输入框样式 */}
                        <form onSubmit={(e)=>{e.preventDefault();handleSubmit(false)}} className="flex items-end w-full max-w-2xl mx-auto p-2 bg-white/95 backdrop-blur-lg rounded-3xl transition-all duration-300 ease-in-out hover:shadow-xl focus-within:shadow-xl shadow-[0_-8px_25px_rgba(0,0,0,0.08),inset_0_1px_0_rgba(255,255,255,0.8)]">
                            ...
                            <div className="flex items-center flex-shrink-0 ml-1">
                                {!showSendButton ? (
                                    ...
                                    /* [代码修改] 应用新的发送按钮样式 */
                                    <button type="submit" className="w-10 h-10 flex items-center justify-center bg-primary text-white rounded-full shadow-lg shadow-primary/40 hover:bg-blue-700 disabled:opacity-50 transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95" disabled={isLoading}>
