// /components/AiChatAssistant.js - v37: (完整版 - 修复核心Bug, 重构交互)
...
const AiChatAssistant = ({ onClose, isFullScreenMode = false }) => {
    ...
    useEffect(() => { setIsMounted(true); try { let finalSettings = { ...DEFAULT_SETTINGS }; const savedSettings = localStorage.getItem('ai_assistant_settings_v22_final'); if (savedSettings) { const parsed = JSON.parse(savedSettings); parsed.prompts = parsed.prompts.map(p => ({ ...p, model: p.model || DEFAULT_SETTINGS.selectedModel, ttsVoice: p.ttsVoice || 'zh-CN-XiaoxiaoMultilingualNeural', avatarUrl: p.avatarUrl || '' })); finalSettings = { ...DEFAULT_SETTINGS, ...parsed }; setSettings(finalSettings); } const savedConversations = localStorage.getItem('ai_assistant_conversations_v22_final'); const parsedConvs = savedConversations ? JSON.parse(savedConversations) : []; setConversations(parsedConvs); if (finalSettings.startWithNewChat || parsedConvs.length === 0) { createNewConversation(finalSettings.currentPromptId); } else { setCurrentConversationId(parsedConvs[0].id); } } catch (e) { createNewConversation(DEFAULT_SETTINGS.currentPromptId); } }, []);

    const currentConversation = useMemo(() => conversations.find(c => c.id === currentConversationId), [conversations, currentConversationId]);

    useEffect(() => { const handleClickOutside = (event) => { if (optionsContainerRef.current && !optionsContainerRef.current.contains(event.target)) { setShowMoreMenu(false); setShowModelSelector(false); } }; document.addEventListener('mousedown', handleClickOutside); return () => document.removeEventListener('mousedown', handleClickOutside); }, []);
    ...
    useEffect(() => {
        ...
        if (currentMessageCount > lastKnownCount && lastMessage?.role === 'ai' && lastMessage?.content) {
            setTimeout(() => {
    ...
    }, [currentConversation, settings.autoRead, isMounted]);

    const createNewConversation = (promptId) => { const newId = `conv-${Date.now()}`; const newConv = { id: newId, title: '新的对话', messages: [{ role: 'ai', content: '你好！有什么可以帮助你的吗？' }], promptId: promptId || settings.currentPromptId }; setConversations(prev => [newConv, ...prev]); setCurrentConversationId(newId); };
