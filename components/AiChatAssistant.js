...
// 1. 导入你的题型组件
// import PaiXuTi from './Tixing/PaiXuTi'; // 移除练习题组件

// 2. 创建一个组件映射表
// const componentMap = {
  // PaiXuTi: PaiXuTi // 移除练习题组件映射
};
                    ...
                    <footer className="flex-shrink-0 p-2 sm:p-4 pb-safe bg-gradient-to-t from-white/80 via-white/50 to-transparent dark:from-gray-800/80 dark:via-gray-800/50 z-10">
                        ...
                        <div className="flex items-center justify-center gap-2 mb-2 max-w-3xl mx-auto">
                           ...
                           <div className="relative">
                             <button type="button" className="px-3 py-1.5 bg-white dark:bg-gray-700 rounded-full text-xs text-gray-700 dark:text-gray-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),_0_1px_3px_rgba(0,0,0,0.08)] dark:shadow-none hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200 active:scale-95" title="选择语音识别语言">
                               <i className="fas fa-language mr-1"></i> <span>语言</span>
                             </button>
                             <div className="absolute left-0 mt-2 w-36 bg-white dark:bg-gray-700 rounded-lg shadow-lg border border-gray-200 dark:border-gray-600 z-30 p-2 hidden group-hover:block">
                               {(settings.speechLanguageOptions || [
                                 ...
                                 <button key={lang.value} onClick={() => {
                                   setSettings(s => ({ ...s, speechLanguage: lang.value }));
                                   // 如果正在听，需要停止并重新开始以应用新语言
                                   if (isListening) {
                                     stopListening();
                                     setTimeout(() => startListening(), 100); // 稍微延迟以确保旧的 recognition 实例停止
                                   }
                                 }} className={`block w-full text-left px-3 py-2 text-sm rounded-md ${settings.speechLanguage === lang.value ? 'font-bold text-blue-600 dark:text-blue-400 bg-blue-500/10' : 'hover:bg-gray-100 dark:hover:bg-gray-600'}`}>
                                   {lang.name}
                                 </button>
                               ))}
                             </div>
                        ...
                        <form onSubmit={(e)=>{e.preventDefault();handleSubmit(false)}} className="flex items-end w-full max-w-3xl mx-auto p-2 bg-white dark:bg-gray-700 backdrop-blur-sm rounded-2xl border border-gray-200/80 dark:border-gray-600/80 transition-shadow duration-300 ease-in-out hover:shadow-2xl focus-within:shadow-2xl shadow-[0_-10px_25px_rgba(0,0,0,0.05),_0_-5px_10px_rgba(0,0,0,0.04)]">
                            ...
                            <textarea ref={textareaRef} value={userInput} onChange={(e) => setUserInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSubmit(false); } }} placeholder={isListening ? "正在聆听..." : "输入消息..."} className="flex-1 bg-transparent focus:outline-none text-gray-800 dark:text-gray-200 text-base resize-none overflow-hidden mx-2 py-1.5 leading-6 max-h-36 placeholder-gray-500 dark:placeholder-gray-400" rows="1" style={{minHeight:'2.5rem'}} readOnly={isListening} />
                            <div className="flex items-center flex-shrink-0 ml-1">
                                {!showSendButton ? ( <button type="button" onClick={isListening ? stopListening : startListening} className={`w-12 h-12 flex items-center justify-center rounded-full transition-colors ${isListening ? 'text-white bg-red-500 animate-pulse' : 'text-blue-500 hover:bg-blue-500/10 dark:hover:bg-white/10'}`} title="语音输入"> <i className="fas fa-microphone text-2xl"></i> </button> ) : ( <button type="submit" className="w-12 h-12 flex items-center justify-center bg-blue-600 text-white rounded-full shadow-lg shadow-blue-500/30 hover:bg-blue-700 disabled:opacity-50 transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95" disabled={isLoading}> <i className="fas fa-arrow-up text-2xl"></i> </button> )}
                            </div>
                        </form>
                    </footer>
                </div>
