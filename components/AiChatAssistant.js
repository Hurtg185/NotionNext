...
const AiChatAssistant = ({ onClose }) => {
    ...
    // --- 保存设置和对话 ---
    useEffect(() => { if (isMounted) { safeLocalStorageSet('ai_assistant_settings_v61', JSON.stringify(settings)); safeLocalStorageSet('ai_assistant_conversations_v61', JSON.stringify(conversations)); } }, [settings, conversations, isMounted]);
    ...
    const handleSaveSettings = (newSettings) => { setSettings(newSettings); setShowSettings(false); safeLocalStorageSet('ai_assistant_settings_v61', JSON.stringify(newSettings)); };
