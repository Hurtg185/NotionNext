// /components/AiChatAssistant.js - 终极版 v12：修复全屏模式，增加快捷提示词切换
...
const SettingsModal = ({ settings, onSave, onClose }) => {
    ...
    const geminiTtsVoices = [ 'Zephyr', 'Puck', 'Charon', 'Kore', 'Fenrir', 'Leda' ];
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="mb-4 pb-4 border-b dark:border-gray-700">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Google Gemini API 密钥</label>
                ...
                {/* 聊天模型 */}
                <div className="mb-4 pb-4 border-b dark:border-gray-700">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">AI 聊天模型</label>
                ...
                {/* TTS 设置 */}
                <div className="mb-4 pb-4 border-b dark:border-gray-700">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">TTS 引擎</label>
                    <select value={tempSettings.ttsEngine} onChange={(e) => handleChange('ttsEngine', e.target.value)} className="w-full px-3 py-2 bg-gray-100 dark:bg-gray-700 border rounded-md">
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <select value={tempSettings.ttsModel} onChange={(e) => handleChange('ttsModel', e.target.value)} className="w-full px-3 py-2 bg-gray-100 dark:bg-gray-800 border rounded-md">
                                    {ttsModels.map(m => <option key={m.value} value={m.value}>{m.name}</option>)}
                                </select>
                            </div>
                            <div>
                                ...
                                <select value={tempSettings.ttsVoice} onChange={(e) => handleChange('ttsVoice', e.target.value)} className="w-full px-3 py-2 bg-gray-100 dark:bg-gray-800 border rounded-md">
                                    {geminiTtsVoices.map(v => <option key={v} value={v}>{v}</option>)}
                                </select>
                            </div>
                        </div>
                    </div>
...
const AiChatAssistant = () => {
    ...
    useEffect(() => {
        setIsMounted(true);
        try {
            const savedSettings = localStorage.getItem('ai_assistant_settings_v12_final');
            if (savedSettings) {
    ...
    useEffect(() => {
        if (isMounted) {
            localStorage.setItem('ai_assistant_settings_v12_final', JSON.stringify(settings));
        }
    }, [settings, isMounted]);
    ...
    useEffect(() => {
        ...
        if (isFullScreen) {
            body.style.overflow = 'hidden'; // 禁止页面滚动
        } else {
    ...
    const startListening = useCallback(() => { /* ... (此处省略未改动的语音输入代码) ... */ }, [settings.speechLanguage]);
    const stopListening = useCallback(() => { /* ... (此处省略未改动的语音输入代码) ... */ }, []);
        ...
        <div className={`flex flex-col bg-white dark:bg-gray-800 ${containerClasses}`} style={containerStyle}>
            ...
            <div className="flex items-center justify-between py-1 px-2 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-b dark:border-gray-700 shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-bold text-gray-800 dark:text-white">AI 中文老师 <span className="text-xs text-gray-500 font-normal">({currentPromptName})</span></h2>
                </div>
            ...
            <div className="p-2 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-t dark:border-gray-700 shrink-0">
                ...
                {isLoading ? (
                    ...
                    <div className="relative flex items-end gap-2">
                        ...
                        <button type="button" onClick={handleSubmit} className="p-3 bg-primary text-white rounded-full hover:bg-blue-dark disabled:opacity-50 shrink-0" disabled={!userInput.trim() && !selectedImage}><i className="fas fa-arrow-up"></i></button>
                    </div>
                )}
            </div>
