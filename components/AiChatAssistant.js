// /components/AiChatAssistant.js - v65.12 (Complete, Unabridged Final Version)
...
const safeLocalStorageGet = (key) => { if (typeof window !== 'undefined') { try { return localStorage.getItem(key); } catch (e) { console.error('Failed to read from localStorage', e); return null; } } return null; };
const safeLocalStorageSet = (key, value) => { if (typeof window !== 'undefined') { try { localStorage.setItem(key, value); } catch (e) { console.error('Failed to write to localStorage', e); } } };
const safeLocalStorageRemove = (key) => { if (typeof window !== 'undefined') { try { localStorage.removeItem(key); } catch (e) { console.error('Failed to remove from localStorage', e); } } };
...
const DEFAULT_PROMPTS = [ { id: 'default-grammar-correction', name: '纠正中文语法', content: '你是一位专业的、耐心的中文老师，请纠正我发送的中文句子中的语法和用词错误，并给出修改建议和说明。', model: 'gemini-2.5-flash', avatarUrl: '' }, { id: 'explain-word', name: '解释中文词语', content: '你是一位专业的中文老师，请用简单易懂的方式解释我发送的中文词语，并提供几个例子。', model: 'gemini-1.5-pro-latest', avatarUrl: '' }, { id: 'translate-myanmar', content: '你是一位专业的翻译助手，请将我发送的内容在中文和缅甸语之间进行互译。', model: 'gemini-2.5-flash', avatarUrl: '' } ];
...
const DEFAULT_SETTINGS = {
    apiKeys: [], activeApiKeyId: '', chatModels: CHAT_MODELS_LIST, selectedModel: 'gemini-2.5-flash',
...
// --- 子组件 (全部内联到 AiChatAssistant.js 中以确保作用域正确) ---
...
const AiChatAssistant = ({ onClose }) => {
        ...
        const renderConversationItem = (conv) => (
            <div key={conv.id} className={`group flex items-center p-2 rounded-md cursor-pointer transition-all duration-200 ${currentId === conv.id ? 'bg-primary/10' : 'hover:bg-gray-200/50'}`} onClick={() => onSelect(conv.id)}>
                <div className="flex-grow truncate" onDoubleClick={(e) => { e.stopPropagation(); handleRename(conv.id, conv.title); }}>
                    {editingId === conv.id ? (
                        <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} onBlur={() => handleSaveRename(conv.id)} onKeyDown={(e) => e.key === 'Enter' && handleSaveRename(conv.id)} className="w-full bg-transparent p-0 border-b border-gray-400" autoFocus />
                    ) : (
                        <span className={`text-sm ${currentId === conv.id ? 'text-primary font-semibold' : ''}`}>{conv.title}</span>
                    )}
                </div>
    ...
    const SubPageWrapper = ({ title, onBack, children }) => (
        <div className="p-4 sm:p-6 h-full flex flex-col bg-white">
            <div className="flex items-center justify-between mb-4 shrink-0">
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-center flex-grow text-gray-800">{title}</h3>
    ...
    useEffect(() => {
        const initializeApp = async () => {
            ...
            const savedSettings = safeLocalStorageGet('ai_assistant_settings_v65');
            if (savedSettings) {
                try {
                    const parsed = JSON.parse(savedSettings);
                    if (parsed.searchMode) delete parsed.searchMode;
                    if (parsed.thirdPartyTtsConfig?.rate !== undefined) delete parsed.thirdPartyTtsConfig.rate;
                    if (parsed.thirdPartyTtsConfig?.pitch !== undefined) delete parsed.thirdPartyTtsConfig.pitch;
            ...
            const savedConversations = safeLocalStorageGet('ai_assistant_conversations_v65');
    ...
    const createNewConversation = useCallback((promptId, isInitial = false) => {
        const newId = generateSimpleId('conv');
        const newConv = { id: newId, title: '新的对话', messages: [{ role: 'ai', content: '你好！有什么可以帮助你的吗？', timestamp: Date.now() }], promptId: promptId || settings.currentPromptId };
        if (isInitial) {
            lastAutoReadMessageId.current = newConv.messages[0].timestamp;
        }
        setConversations(prev => [newConv, ...prev]);
        setCurrentConversationId(newId);
    }, [settings.currentPromptId]);
    ...
    return (
        <div className="w-full h-full flex flex-col bg-gray-100 text-gray-800">
            ...
            <div className="relative flex flex-1 min-h-0">
                <ChatSidebar isOpen={isSidebarOpen} conversations={conversations} currentId={currentConversationId} onSelect={handleSelectConversation} onDelete={handleDeleteConversation} onRename={handleRenameConversation} onNew={() => createNewConversation(settings.currentPromptId)} prompts={settings.prompts} settings={settings} />
                ...
                <div className="flex-1 flex flex-col h-full min-w-0 z-10">
                    ...
                    <footer className="flex-shrink-0 px-4 pt-2 pb-safe bg-gradient-to-t from-gray-100/50 via-gray-100/30 to-transparent z-10">
                        ...
                        <div className="flex items-center justify-center gap-2 mb-2 max-w-2xl mx-auto">
                             <button onClick={() => createNewConversation(settings.currentPromptId)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/70 backdrop-blur-sm border border-gray-300/50 rounded-full text-gray-600 text-xs hover:bg-white shadow-sm" title="新对话"> <i className="fas fa-plus"></i> </button>
