// /components/AiChatAssistant.js - v45 (最终整合版 - 基于v40 + 高端激活界面 + 快速试用期 + 所有Bug修复)
...
const MessageBubble = ({ msg, settings, isLastAiMessage, onRegenerate }) => {
    ...
    const userBubbleClass = 'bg-primary text-white rounded-br-lg';
    ...
    return (
        <div className={`flex items-end gap-2.5 my-2 ${isUser ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`p-3 rounded-2xl text-left flex flex-col ${isUser ? userBubbleClass : aiBubbleClass}`} style={{ maxWidth: '85%' }}>
                ...
                {!isUser && msg.content && ( <div className="flex items-center gap-2 mt-2 -mb-1 text-gray-500 dark:text-gray-400"> <AiTtsButton text={msg.content} ttsSettings={settings} /> <button onClick={() => navigator.clipboard.writeText(msg.content)} className="p-2 rounded-full hover:bg-black/10 dark:hover:bg-white/10" title="复制"><i className="fas fa-copy"></i></button> {isLastAiMessage && ( <button onClick={onRegenerate} className="p-2 rounded-full hover:bg-black/10 dark:hover:bg-white/10" title="重新生成"><i className="fas fa-sync-alt"></i></button> )} </div> )}
            </div>
...
const SettingsModal = ({ settings, onSave, onClose }) => {
    ...
    const handleDeleteModel = (indexToDelete) => { if ((tempSettings.chatModels || []).length <= 1) { alert("至少需要保留一个模型。"); return; } if (!window.confirm('确定删除此模型吗？')) return; const newModels = (tempSettings.chatModels || []).filter((_, i) => i !== indexToDelete); handleChange('chatModels', newModels); };
    const handleModelSettingChange = (index, field, value) => { const newModels = (tempSettings.chatModels || []).map((m, i) => i === index ? { ...m, [field]: value } : m); handleChange('chatModels', newModels); };
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                    <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md">
                        <button onClick={() => setPromptsCollapsed(!promptsCollapsed)} className="w-full flex justify-between items-center text-left"><h4 className="text-md font-semibold">自定义提示词管理</h4><i className={`fas fa-chevron-down transition-transform duration-200 ${!promptsCollapsed ? 'rotate-180' : ''}`}></i></button>
                        {!promptsCollapsed && (
                             <div className="mt-4">
                                <div className="space-y-2 mb-4 max-h-48 overflow-y-auto">
                                    {(tempSettings.prompts || []).map(p => (
                                        <div key={p.id} className="p-3 bg-gray-100 dark:bg-gray-700 rounded-md">
                                            <div className="flex items-center justify-between"><label className="flex items-center flex-grow cursor-pointer"><input type="radio" name="currentPrompt" checked={tempSettings.currentPromptId === p.id} onChange={() => handleChange('currentPromptId', p.id)} className="mr-2 text-primary" /><input type="text" value={p.name} onChange={(e) => handlePromptSettingChange(p.id, 'name', e.target.value)} className="font-medium bg-transparent w-full" /></label><button onClick={() => handleDeletePrompt(p.id)} className="p-1 ml-2 text-sm text-red-500 rounded"><i className="fas fa-trash"></i></button></div>
                                            ...
                                            <div className="mt-2 space-y-2 text-sm">
                                                <div className="flex items-center gap-2"><label className="shrink-0">模型:</label><select value={p.model || settings.selectedModel} onChange={(e) => handlePromptSettingChange(p.id, 'model', e.target.value)} className="w-full px-2 py-1 bg-white dark:bg-gray-800 border rounded-md text-xs">{(tempSettings.chatModels || []).map(m => <option key={m.value} value={m.value}>{m.name}</option>)}</select></div>
                                                <div className="flex items-center gap-2"><label className="shrink-0">声音:</label><select value={p.ttsVoice || settings.thirdPartyTtsVoice} onChange={(e) => handlePromptSettingChange(p.id, 'ttsVoice', e.target.value)} className="w-full px-2 py-1 bg-white dark:bg-gray-800 border rounded-md text-xs">{microsoftTtsVoices.map(v => <option key={v.value} value={v.value}>{v.name}</option>)}</select></div>
                                                <div className="flex items-center gap-2"><label className="shrink-0">头像:</label><input type="text" value={p.avatarUrl || ''} onChange={(e) => handlePromptSettingChange(p.id, 'avatarUrl', e.target.value)} placeholder="输入头像图片URL" className="w-full px-2 py-1 bg-white dark:bg-gray-800 border rounded-md text-xs" /></div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                     ...
                     <div className="text-sm">
                         <button onClick={() => setApiKeyInfoCollapsed(!apiKeyInfoCollapsed)} className="w-full flex justify-between items-center text-left text-gray-600 dark:text-gray-400">
                            <span><i className="fas fa-info-circle mr-2"></i>使用说明：如何获取 API 密钥？</span><i className={`fas fa-chevron-down transition-transform text-xs ${!apiKeyInfoCollapsed ? 'rotate-180' : ''}`}></i>
                         </button>
                         {!apiKeyInfoCollapsed && (
...
const AiChatAssistant = ({ onClose, isFullScreenMode = false }) => {
    ...
    const getDeviceId = async () => {
        try {
            ...
            return 'fallback-device-id-' + Date.now(); // 提供一个备用ID
        }
    };
            ...
            try {
                ...
                if (storedKey) {
                    ...
                    if (response.ok && data.success) {
                        ...
                        if (data.keyType === 'trial') { const expiry = new Date(data.activatedAt + (data.durationSeconds || 0) * 1000); setTrialExpiryInfo(`试用中, 剩余: ${Math.round((expiry.getTime() - Date.now()) / 60000)}分钟`); } else { setTrialExpiryInfo('永久授权'); }
                        activated = true;
                    } else {
                ...
                if (!activated) {
                    ...
                    if (trialResponse.ok && trialData.success) {
                        ...
                        const expiry = new Date(Date.now() + (trialData.durationSeconds || 0) * 1000); setTrialExpiryInfo(`试用中, 剩余: ${Math.round((expiry.getTime() - Date.now()) / 60000)}分钟`);
                    } else {
        ...
        return (
            <div className={containerClasses} style={containerStyle}>
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-lg animate-fade-in">
                    <div className="relative w-full max-w-md overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 to-black border border-gray-700/50 shadow-2xl animate-scale-in">
                        <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500"></div>
                        {isFullScreenMode && ( <button onClick={onClose} className="absolute top-4 right-4 z-10 p-2 text-gray-400 hover:text-white transition-colors duration-300"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button> )}
                        <div className="p-8">
                            <div className="mb-6 w-full animate-slide-in-up delay-100">
                                <img src="/images/jihuomatu.jpg" alt="课程介绍" className="w-full h-auto rounded-md mb-4 shadow-md" />
                                <h2 className="text-2xl font-bold mb-2 text-center bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400">报名中文课程</h2>
                                <p className="text-gray-400 text-sm mb-2 text-center">【课程介绍】结合中缅教学方案，高效学习中文，价格比大部分缅甸机构更优惠！</p>
                                <p className="text-gray-400 text-sm mb-2 text-center">【地址】仰光某区，欢迎线下咨询！</p>
                                <p className="text-xl font-bold text-green-500 mb-4 text-center">【优惠价格】AI助手套餐：$50 / 月（原价$80）</p>
                                <p className="text-gray-400 text-sm mb-4 text-center">请通过以下方式联系我们，获取专属学习方案：</p>
                            </div>
                            <div className="space-y-3 w-full mb-6">
                                <a href="https://t.me/yourtelegramid" target="_blank" rel="noopener noreferrer" className="block w-full text-white bg-blue-500 hover:bg-blue-600 rounded-lg py-3 text-lg font-semibold flex items-center justify-center space-x-2 transition-all duration-300 transform hover:-translate-y-1 shadow-lg animate-slide-in-up delay-200"><i className="fab fa-telegram-plane"></i> <span>Telegram 联系</span></a>
                                <a href="https://line.me/ti/p/@yourlineid" target="_blank" rel="noopener noreferrer" className="block w-full text-white bg-green-500 hover:bg-green-600 rounded-lg py-3 text-lg font-semibold flex items-center justify-center space-x-2 transition-all duration-300 transform hover:-translate-y-1 shadow-lg animate-slide-in-up delay-300"><i className="fab fa-line"></i> <span>Line 联系</span></a>
                                <a href="viber://chat?number=+959XXXXXXXX" target="_blank" rel="noopener noreferrer" className="block w-full text-white bg-purple-500 hover:bg-purple-600 rounded-lg py-3 text-lg font-semibold flex items-center justify-center space-x-2 transition-all duration-300 transform hover:-translate-y-1 shadow-lg animate-slide-in-up delay-400"><i className="fab fa-viber"></i> <span>Viber 联系</span></a>
                                <a href="https://www.facebook.com/share/1FSxz6cm2Z" target="_blank" rel="noopener noreferrer" className="block w-full text-white bg-blue-700 hover:bg-blue-800 rounded-lg py-3 text-lg font-semibold flex items-center justify-center space-x-2 transition-all duration-300 transform hover:-translate-y-1 shadow-lg animate-slide-in-up delay-500"><i className="fab fa-facebook"></i> <span>Facebook 主页</span></a>
                            </div>
                            {activationError && <p className="text-red-500 text-sm mb-4 animate-shake">{activationError}</p>}
                            <form onSubmit={handleActivate} className="space-y-4 w-full">
                                <div className="relative animate-slide-in-up delay-600">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><svg className="h-5 w-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg></div>
                                    <input type="text" value={activationKey} onChange={(e) => setActivationKey(e.target.value)} placeholder="在此输入您的激活码" className="w-full pl-10 pr-4 py-3 bg-gray-800/50 border border-gray-700/50 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-transparent transition-all duration-300"/>
                                </div>
                                <button type="submit" disabled={isActivating} className="w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-500 hover:to-purple-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transition-all duration-300 transform hover:-translate-y-1 shadow-lg animate-slide-in-up delay-700">
