...
const getLevelPrice = (level) => {
  const prices = { 1: "10,000 Ks", 2: "15,000 Ks", 3: "20,000 Ks", 'SP': "30,000 Ks" };
...
const hskData = [
  {
    level: 1, title: '入门 (Intro)', description: '掌握最常用词语和基本语法', descBurmese: 'အသုံးအများဆုံး စကားလုံးများနှင့် သဒ္ဒါ',
  ...
  {
    level: 2, title: '基础 (Basic)', description: '就熟悉的日常话题进行交流', descBurmese: 'နေ့စဉ်သုံး စကားပြောများ',
...
const MembershipModal = ({ isOpen, onClose, targetLevel }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-sm bg-white rounded-[2rem] shadow-2xl overflow-hidden p-6" >
        ...
        <div className="text-center mt-2">
          ...
          <p className="text-slate-500 font-medium text-sm mt-1 mb-5">
            {isSpoken ? "地道场景、谐音助记与 AI 评测" : "完整视频讲解与练习题"}<br/>
          ...
          <a href={FB_CHAT_LINK} target="_blank" rel="noopener noreferrer" className="w-full py-3.5 bg-[#0084FF] text-white rounded-xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-all shadow-lg hover:bg-blue-600" >
            <MessageCircle size={20} fill="currentColor" /> ဆက်သွယ်ရန် (Contact)
          </a>
        </div>
      </motion.div>
    </div>
...
const HskCard = ({ level, onVocabularyClick, onShowMembership }) => {
  ...
  const handleLessonClick = (e, lesson) => {
    ...
    if (!isFree) { e.preventDefault(); onShowMembership(level.level); return; }
    router.push(`/hsk/${level.level}/lessons/${lesson.id}`);
  };

  return (
    <motion.div whileTap={{ scale: 0.995 }} className="flex flex-col bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden relative z-10" >
      ...
      <div className="px-4 pb-5 pt-1 flex flex-col gap-3">
        {level.lessons.length > 3 && (
          <button onClick={() => setIsExpanded(!isExpanded)} className="w-full py-2.5 flex items-center justify-center text-xs font-bold text-slate-500 gap-1 bg-slate-50 border border-slate-100 rounded-xl hover:bg-slate-100 transition-colors active:scale-95" >
            {isExpanded ? 'See Less' : `View All ${level.lessons.length} Lessons`} 
            {isExpanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
          </button>
        )}
        <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onVocabularyClick(level); }} className="relative z-20 w-full py-3 flex items-center justify-center gap-2 bg-gradient-to-r from-pink-50 to-rose-50 text-rose-500 rounded-xl font-black text-xs active:scale-95 transition-all border border-pink-100 hover:shadow-sm" >
          <BookOpen size={14} /> <span>核心生词</span> <span className="text-[10px] opacity-70 font-normal ml-1">(ဝေါဟာရများ)</span>
        </button>
      </div>
    </motion.div>
  );
        ...
        {pinyinMain.map((item) => (
          <Link key={item.id} href={item.href} passHref>
            <a className={`flex flex-col items-center justify-center py-3 rounded-2xl ${item.bg} active:scale-95 transition-transform`}>
              <div className="mb-1 bg-white p-1.5 rounded-full shadow-sm"><item.icon size={16} className={item.color} /></div>
              <span className="text-[10px] font-bold text-slate-600 text-center leading-tight">{item.title}<br/><span className="text-[8px] opacity-70">{item.sub}</span></span>
            </a>
          </Link>
        ))}
      </div>

      <button onClick={() => router.push('/pinyin/tips')} className="w-full flex items-center justify-between px-4 py-3 bg-gradient-to-r from-orange-50 to-amber-50 rounded-2xl border border-orange-100/50 active:scale-95 transition-transform group">
        <div className="flex items-center gap-3">
          <div className="p-1.5 bg-white rounded-full text-orange-500 shadow-sm shrink-0"><Lightbulb size={16} fill="currentColor" /></div>
          <div className="text-left"><span className="block text-xs font-black text-slate-700">发音技巧 (Tips)</span><span className="block text-[10px] text-slate-500 font-medium">အသံထွက်နည်းလမ်းများ</span></div>
        </div>
        <ChevronRight size={16} className="text-orange-300" />
      </button>
      
      <div className="grid grid-cols-2 gap-3">
        <button onClick={onOpenCollection} className="flex flex-col items-center justify-center py-3 bg-white border border-blue-100 rounded-2xl shadow-sm active:scale-95 transition-transform">
          <div className="flex items-center gap-1.5 mb-1 text-blue-600"><Star size={14} fill="currentColor"/><span className="text-xs font-black">单词收藏</span></div>
          <span className="text-[9px] text-slate-400">မှတ်ထားသော စာလုံး</span>
        </button>
        <button onClick={onOpenSpokenCollection} className="flex flex-col items-center justify-center py-3 bg-white border border-emerald-100 rounded-2xl shadow-sm active:scale-95 transition-transform">
           <div className="flex items-center gap-1.5 mb-1 text-emerald-600"><Volume2 size={14} fill="currentColor"/><span className="text-xs font-black">口语收藏</span></div>
...
export default function HskPageClient() {
  ...
  const handleCollectionClick = useCallback(() => {
    ...
    const favoriteWords = allWords.filter(word => savedIds.some(savedId => String(savedId) === String(word.id)));
    if (favoriteWords.length === 0) { alert("No saved words yet!\nမှတ်ထားသော စာလုံး မရှိသေးပါ"); return; }
  ...
  // SSR 阶段返回一个干净的结构，避免 Node.js 访问 self
  if (!mounted) {
    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans text-slate-900 pb-20 relative overflow-x-hidden max-w-md mx-auto shadow-2xl shadow-slate-200">
      ...
      <header className="relative pt-4 px-4 pb-1 z-10">
        ...
        <div className="bg-white rounded-[1.8rem] p-4 shadow-xl shadow-slate-200/60 border border-slate-50">
          <PinyinSection onOpenCollection={handleCollectionClick} onOpenSpokenCollection={handleSpokenCollectionClick} />
        </div>
      </header>
      ...
      <div className="px-4 mt-4">
        <div onClick={handleSpokenGeneralClick} className="block relative h-28 w-full rounded-3xl overflow-hidden shadow-lg shadow-emerald-100 group active:scale-[0.98] transition-all cursor-pointer" >
          <img src="https://images.unsplash.com/photo-1543269865-cbf427effbad?w=800&q=80" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt="Oral Chinese" />
          <div className="absolute inset-0 bg-gradient-to-r from-emerald-600/90 via-emerald-600/40 to-transparent flex flex-col justify-center px-6">
            <div className="flex items-center gap-2 text-emerald-100 mb-1"><Headphones size={16} /><span className="text-[10px] font-bold uppercase tracking-widest">Oral Practice</span></div>
          ...
          <div className="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/20 backdrop-blur rounded-full flex items-center justify-center text-white"><ChevronRight size={20} /></div>
        </div>
      </div>

      <div className="max-w-2xl mx-auto px-4 relative z-10 mt-6 space-y-4">
        <div className="flex items-center gap-2 px-1 opacity-70"><BookText size={14} className="text-slate-500" /><h2 className="text-xs font-black text-slate-600 uppercase tracking-wider">System Courses (သင်ရိုး)</h2></div>
        <div className="grid grid-cols-1 gap-5 pb-10">
          {hskData.map(level => (
            <HskCard key={level.level} level={level} onVocabularyClick={handleVocabularyClick} onShowMembership={(l) => setMembership({ open: true, level: l })} />
          ))}
        </div>
      </div>

      <AnimatePresence>
        {membership.open && <MembershipModal isOpen={membership.open} onClose={() => setMembership({ open: false, level: null })} targetLevel={membership.level} />}
      </AnimatePresence>

      <WordCard isOpen={isCardViewOpen} words={activeHskWords || []} onClose={() => router.push(router.pathname, undefined, { shallow: true })} progressKey={activeLevelTag || 'hsk-vocab'} />
    </div>
  );
}
