...
const XuanZeTi = (props) => {
  ...
  // --- 关键修复点：识别 content 嵌套层级 ---
  const data = rawData.content || rawData;

  const rawQuestion = props.question || data.question || {};
  const rawOptions = props.options || data.options || [];
  const rawCorrectAnswer = props.correctAnswer || data.correctAnswer || [];
  const explanationText = props.explanation || data.explanation || "";
      ...
      <div className="xzt-container">
        ...
        <div className="xzt-scroll-area">
          <div className="scene-wrapper">
            ...
            <div className="bubble-container">
              ...
              <div className="bubble-content">
                ...
                <div className={`bubble-audio-btn ${isPlaying ? 'playing' : ''}`} onClick={(e) => handleTitlePlay(e, false)}>
                  <FaVolumeUp />
                </div>
              </div>
            </div>
          ...
          <div className={`xzt-options-grid ${hasOptionImages ? 'grid-images' : ''}`}>
            {orderedOptions.map(opt => {
              ...
              let cardClass = "";
              if (opt.hasImage) cardClass += " card-with-image";
        ...
        {!isSubmitted && (
          <div className="bottom-submit-area">
            <button className="submit-btn" onClick={handleSubmit} disabled={!selectedId}>
              တင်သွင်းသည်
            </button>
          </div>
        )}

        <div className={`overlay-backdrop ${isSubmitted ? 'show' : ''}`} />
        ...
        <div className={`settings-overlay ${showSettings ? 'show' : ''}`} onClick={() => setShowSettings(false)}>
          <div className="settings-panel" onClick={e => e.stopPropagation()}>
            ...
            <div className="setting-group">
              ...
              <select className="setting-select" value={settings.voice} onChange={(e) => setSettings({...settings, voice: e.target.value})}>
            ...
            <div className="setting-group">
              ...
              <div className="speed-options">
                {SPEED_OPTIONS.map(s => (
                  <button key={s} className={`speed-btn ${settings.speed === s ? 'active' : ''}`} onClick={() => setSettings({...settings, speed: s})}>
                    {s}x
                  </button>
                ))}
              </div>
            </div>
            <button className="close-settings-btn" onClick={() => setShowSettings(false)}>Done</button>
          </div>
        </div>
      </div>
    </>
