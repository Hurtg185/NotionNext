...
const cssStyles = `
  ...
  .scene-wrapper {
    ...
    margin-bottom: 24px; position: relative; gap: 20px;
  }

  .teacher-img {
    height: 160px; width: auto; object-fit: contain;
    z-index: 2; mix-blend-mode: multiply; 
    filter: contrast(1.05); flex-shrink: 0;
  }

  .bubble-container {
    flex: 1; max-width: 280px; background: var(--white);
    border-radius: 18px; padding: 14px 18px;
    box-shadow: 0 8px 25px rgba(0,0,0,0.06); position: relative;
    z-index: 1; margin-bottom: 35px; border: 1px solid rgba(255,255,255,0.8);
  }
  ...
  .bubble-audio-btn {
    align-self: flex-end; width: 28px; height: 28px; background: #eef2ff;
    border-radius: 50%; display: flex; align-items: center; justify-content: center;
    color: var(--primary-color); cursor: pointer; font-size: 0.8rem;
  }
  ...
  .question-ref-img {
    ...
    border-radius: 8px; margin-bottom: 5px; background: #f8fafc; display: block;
  }
  ...
  .xzt-option-card {
    ...
    border: 2px solid transparent; transition: all 0.2s cubic-bezier(0.2, 0.8, 0.2, 1);
    overflow: hidden;
  }
  ...
  .opt-img { width: 45px; height: 45px; border-radius: 8px; object-fit: cover; margin-right: 12px; flex-shrink: 0; }
  .card-with-image .opt-img { width: 100%; height: 130px; margin-right: 0; border-radius: 0; object-fit: cover; }

  .opt-content { flex: 1; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; }
  ...
  .status-icon { font-size: 1.4rem; position: absolute; right: 16px; }
  ...
  .submit-btn {
    pointer-events: auto; background: var(--primary-color); color: white; border: none;
    padding: 14px 70px; border-radius: 99px; font-size: 1.1rem; font-weight: 700;
    box-shadow: 0 8px 20px -4px rgba(99, 102, 241, 0.4); transition: transform 0.2s;
  }
  ...
  .explanation-sheet {
    ...
    padding: 32px 24px 50px 24px; display: flex; flex-direction: column; gap: 20px;
  }
  .explanation-sheet.show { transform: translateY(0); }
  .explanation-sheet.is-right { background: #f0fdf4; border-top: 1px solid var(--success-color); }
  .explanation-sheet.is-wrong { background: #fef2f2; border-top: 1px solid var(--error-color); }

  .result-badge {
    padding: 8px 24px; border-radius: 30px; font-size: 1.2rem; font-weight: 800;
    display: flex; align-items: center; gap: 8px; background: white; justify-content: center;
  }
  .is-right .result-badge { color: var(--success-color); }
  .is-wrong .result-badge { color: var(--error-color); }

  .explanation-box { background: rgba(255, 255, 255, 0.8); border-radius: 16px; padding: 20px; text-align: center; }
  ...
  .next-btn {
    ...
    padding: 18px; border-radius: 18px; font-size: 1.15rem; font-weight: 700;
  ...
  .speed-btn { flex: 1; padding: 8px; border-radius: 8px; border: 1px solid #e2e8f0; cursor: pointer; background: white; }
...
const XuanZeTi = (props) => {
  ...
  const questionImage = rawQuestion.imageUrl || null;
  ...
  return (
    <div className="xzt-container">
      ...
      <div className="xzt-scroll-area">
        ...
        <div className={`xzt-options-grid ${orderedOptions.some(o => o.hasImage) ? 'grid-images' : ''}`}>
          {orderedOptions.map(opt => {
            const sid = String(opt.id);
            const isSel = selectedIds.includes(sid);
            const isCorrectOpt = correctIds.includes(sid);
            
            let cardClass = opt.hasImage ? " card-with-image" : "";
            if (isSubmitted) {
              cardClass += " disabled";
            ...
            return (
              <div key={opt.id} className={`xzt-option-card ${cardClass}`} onClick={() => handleCardClick(opt)}>
                ...
                {isSubmitted && isCorrectOpt && <FaCheck className="status-icon" style={{color:'#10b981'}} />}
                {isSubmitted && isSel && !isCorrectOpt && <FaTimes className="status-icon" style={{color:'#ef4444'}} />}
              </div>
            );
          })}
        </div>
      ...
      <div className={`explanation-sheet ${isSubmitted ? 'show' : ''} ${isRight ? 'is-right' : 'is-wrong'}`}>
        ...
        <button className="next-btn" onClick={handleNextAction}>နောက်တစ်ပုဒ် <FaArrowRight /></button>
      </div>

      <div className={`settings-overlay ${showSettings ? 'show' : ''}`} onClick={() => setShowSettings(false)}>
        <div className="settings-panel" onClick={e => e.stopPropagation()}>
          <h4 style={{margin:'0 0 20px 0'}}>Playback Settings</h4>
          <div className="speed-options">
            {[0.75, 1.0, 1.25].map(s => (
              <button key={s} className={`speed-btn ${settings.speed === s ? 'active' : ''}`} onClick={() => setSettings({...settings, speed: s})}>{s}x</button>
            ))}
          </div>
          <button style={{marginTop:20, width:'100%', padding:10, borderRadius:8, border:'none', background:'#f1f5f9'}} onClick={() => setShowSettings(false)}>Close</button>
        </div>
      </div>
    </div>
  );
