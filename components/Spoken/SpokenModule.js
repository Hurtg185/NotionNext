...
import { 
  ...
  ChevronDown, ChevronUp, Home, CheckCircle2, BookOpen, LayoutList
} from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const autoSpell = async () => {
        ...
        for (let i = 0; i < chars.length; i++) {
            ...
            // 注意：此处假设你有 R2 原音库，否则可用单字 TTS 代替
...
const SettingsPanel = ({ settings, setSettings, onClose }) => {
  return (
    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed top-16 right-4 z-[2000] bg-white rounded-2xl shadow-2xl border border-slate-100 w-72 overflow-hidden">
          ...
          <div className="space-y-2">
             <div className="flex justify-between items-center">
                ...
                <div onClick={() => setSettings(s => ({...s, zhEnabled: !s.zhEnabled}))} className={`w-8 h-4 rounded-full transition-colors relative cursor-pointer ${settings.zhEnabled ? 'bg-blue-500' : 'bg-slate-200'}`}>
                   <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-transform ${settings.zhEnabled ? 'left-4.5' : 'left-0.5'}`}/>
                </div>
             </div>
             <div className="grid grid-cols-2 gap-2">
                {[{l:'小晓(女)',v:'zh-CN-XiaoxiaoMultilingualNeural'},{l:'云希(男)',v:'zh-CN-YunxiNeural'},{l:'云夏(童)',v:'zh-CN-YunxiaNeural'},{l:'小颜(通)',v:'zh-CN-XiaoyanNeural'}].map(o=>(
                  <button key={o.v} onClick={()=>setSettings(s=>({...s,zhVoice:o.v}))} className={`py-1.5 text-[10px] font-bold rounded border transition-all ${settings.zhVoice===o.v?'bg-blue-50 border-blue-200 text-blue-600':'bg-white text-slate-400'}`}>{o.l}</button>
                ))}
             </div>
             <div className="flex items-center gap-2 pt-1"><span className="text-[10px] text-slate-400">语速</span><input type="range" min="-50" max="50" step="10" value={settings.zhRate} onChange={e=>setSettings(s=>({...s,zhRate:Number(e.target.value)}))} className="flex-1 h-1 bg-slate-100 rounded-lg appearance-none accent-blue-500"/><span className="text-[10px] w-6 text-right font-mono text-slate-400">{settings.zhRate}</span></div>
          </div>
          ...
          <div className="space-y-2">
             <div className="flex justify-between items-center">
                ...
                <div onClick={() => setSettings(s => ({...s, myEnabled: !s.myEnabled}))} className={`w-8 h-4 rounded-full transition-colors relative cursor-pointer ${settings.myEnabled ? 'bg-green-500' : 'bg-slate-200'}`}>
                   <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-transform ${settings.myEnabled ? 'left-4.5' : 'left-0.5'}`}/>
                </div>
             </div>
             <div className="grid grid-cols-2 gap-2">
                {[{l:'Thiha(男)',v:'my-MM-ThihaNeural'},{l:'Nilar(女)',v:'my-MM-NilarNeural'}].map(o=>(
                  <button key={o.v} onClick={()=>setSettings(s=>({...s,myVoice:o.v}))} className={`py-1.5 text-[10px] font-bold rounded border transition-all ${settings.myVoice===o.v?'bg-green-50 border-green-200 text-green-600':'bg-white text-slate-400'}`}>{o.l}</button>
                ))}
             </div>
             <div className="flex items-center gap-2 pt-1"><span className="text-[10px] text-slate-400">语速</span><input type="range" min="-50" max="50" step="10" value={settings.myRate} onChange={e=>setSettings(s=>({...s,myRate:Number(e.target.value)}))} className="flex-1 h-1 bg-slate-100 rounded-lg appearance-none accent-green-500"/><span className="text-[10px] w-6 text-right font-mono text-slate-400">{settings.myRate}</span></div>
          </div>
       </div>
    </motion.div>
  );
...
export default function SpokenModule() {
  ...
  // 播放与交互
  const [settings, setSettings] = useState({ zhVoice: 'zh-CN-XiaoxiaoMultilingualNeural', zhRate: -10, zhEnabled: true, myVoice: 'my-MM-ThihaNeural', myRate: 0, myEnabled: true });
  ...
  useEffect(() => {
    ...
    setIsUnlocked((user.unlocked_levels || '').includes('SP')); // 纯判断逻辑，不弹窗
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    if (view === 'list') {
        ...
        if (latest + winHeight >= docHeight - 200) {
            setVisibleCount(prev => Math.min(prev + PAGE_SIZE, phrases.length));
        }
    }
  });
  ...
  const enterList = (targetSub = null) => {
    ...
    // 重置显示数量，防止一开始渲染太多卡顿
    // 如果是恢复进度，可能需要根据进度计算，简单起见先加载足够多或者分步跳转
    // 这里我们先加载前50条，然后根据 targetSub 跳转
    setVisibleCount(50); 

    setTimeout(() => {
        if (targetSub) {
            // 如果跳转特定目录，需要找到那个 item 的 index
            const targetIndex = phrases.findIndex(p => p.sub === targetSub);
            if (targetIndex !== -1) {
                // 确保数据已渲染出来
                setVisibleCount(Math.max(50, targetIndex + 20));
                setTimeout(() => {
            ...
            if (savedPos) {
                // 粗略估算：假设每条高度200px，根据 scrollPos 计算需要加载多少条
                const estCount = Math.ceil(parseInt(savedPos) / 150) + 20;
                setVisibleCount(Math.max(50, estCount));
                setTimeout(() => window.scrollTo({ top: parseInt(savedPos), behavior: 'auto' }), 50);
            } else {
                window.scrollTo(0, 0);
            }
        }
  ...
  const handleCatalogJump = (sub) => {
    ...
    // 逻辑同 enterList
    const targetIndex = phrases.findIndex(p => p.sub === sub);
    if (targetIndex !== -1) {
        setVisibleCount(Math.max(visibleCount, targetIndex + 20));
        setTimeout(() => {
  ...
  // 数据切片 (防止卡顿的核心)
  const visibleList = phrases.slice(0, visibleCount);
    ...
    <div className="min-h-screen bg-[#F8F9FB] font-sans text-slate-900 max-w-md mx-auto relative shadow-2xl overflow-hidden">
      ...
      {view === 'home' && (
         <div className="min-h-screen bg-white">
            ...
            <div className="relative h-64 overflow-hidden">
               ...
               {/* 网址胶囊 */}
               <div className="absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/20 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/10 z-20 cursor-pointer active:scale-95 transition-transform hover:bg-black/30" onClick={() => window.location.href='https://886.best'}>
            ...
            <div className="px-6 mb-8">
                <button onClick={() => enterList(null)} className="w-full py-3.5 bg-slate-900 text-white rounded-xl font-bold text-sm shadow-xl shadow-slate-200 active:scale-95 transition-transform flex items-center justify-center gap-2">
                  <BookOpen size={18}/> 继续学习
                </button>
            </div>

            {/* 折叠目录 */}
            <div className="px-4 pb-20 space-y-4">
               <h2 className="px-2 text-sm font-black text-slate-400 uppercase tracking-widest">章节目录</h2>
                        ...
                        {expandedCats[cat.name] && (
                           <motion.div initial={{height:0}} animate={{height:'auto'}} exit={{height:0}} className="overflow-hidden bg-slate-50/50">
                              <div className="px-4 pb-4 pt-1 grid grid-cols-2 gap-2">
                                 {cat.subs.map((sub, j) => (
                                    <button key={j} onClick={() => enterList(sub)} className="text-left px-3 py-2 bg-white border border-slate-100 rounded-lg text-xs font-medium text-slate-600 active:scale-95 transition-transform truncate">{sub}</button>
                                 ))}
                              </div>
                           </motion.div>
                        )}
      ...
      {view === 'list' && (
        <div className="min-h-screen pb-32 bg-[#F5F7FA]">
            {/* 网址胶囊 */}
            <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[110] flex items-center gap-1.5 pointer-events-auto cursor-pointer drop-shadow-sm hover:scale-105 transition-transform" onClick={() => window.location.href='https://886.best'}>
                <Globe size={14} className="text-slate-800"/>
                <span className="text-xs font-black text-slate-800 tracking-tight">886.best</span>
            </div>

            {/* 控制栏 */}
            <motion.div initial={{y:0}} animate={{y:isHeaderVisible?0:-80}} className="fixed top-0 left-0 right-0 z-[100] bg-white/90 backdrop-blur-md border-b border-slate-100 h-14 max-w-md mx-auto px-4 flex justify-between items-center">
               <button onClick={goHome} className="p-2 -ml-2 text-slate-500 hover:text-slate-900 active:scale-90 transition-transform"><ChevronLeft size={24} /></button>
               <div className="flex items-center gap-1">
                   <button onClick={() => setShowSettings(!showSettings)} className="p-2 text-slate-400 hover:text-blue-600 active:bg-slate-50 rounded-full"><Settings2 size={20} /></button>
                   <button onClick={() => setShowCatalog(true)} className="p-2 text-slate-600 hover:text-blue-600 active:bg-slate-50 rounded-full"><Menu size={20} /></button>
               </div>
            </motion.div>
            ...
            <AnimatePresence>
                {showCatalog && (
                  <>
                    ...
                    <motion.div initial={{x:'100%'}} animate={{x:0}} exit={{x:'100%'}} transition={{type:'spring', damping:28}} className="fixed inset-y-0 right-0 z-[160] w-[80%] max-w-[280px] bg-white shadow-2xl overflow-y-auto">
                       <div className="p-5">
                          <div className="flex justify-between items-center mb-6"><h2 className="text-lg font-black text-slate-800">快速跳转</h2><button onClick={() => setShowCatalog(false)}><X size={20} className="text-slate-400"/></button></div>
                                     ...
                                     {expandedCats[cat.name] && (
                                       <motion.div initial={{height:0}} animate={{height:'auto'}} exit={{height:0}} className="overflow-hidden">
                                          <div className="pl-3 flex flex-col gap-1 pb-2">
                                            {cat.subs.map((sub, j) => (
                                              <button key={j} onClick={() => handleCatalogJump(sub)} className="text-left py-1.5 text-xs font-medium text-slate-500 hover:text-blue-600 truncate">{sub}</button>
                                            ))}
                                          </div>
                                       </motion.div>
                                     )}
            ...
            <div className="pt-20 px-3 space-y-4">
               {visibleList.map((item, index) => {
                  ...
                  return (
                    <div key={item.id} ref={el => { if(isNewSub) itemRefs.current[item.sub] = el; }}>
                        {isNewSub && <div className="mt-8 mb-3 pl-2 border-l-4 border-blue-500"><h3 className="text-sm font-bold text-slate-800">{item.sub}</h3></div>}
                           ...
                           >
                              ...
                              <div className={isLocked ? 'opacity-30 blur-sm pointer-events-none' : ''}>
                                  ...
                                  <div className="w-full flex justify-center items-center gap-5 pt-3 border-t border-slate-50">
                                     ...
                                     <button onClick={(e) => { e.stopPropagation(); handleSpeech(item); }} className={`w-12 h-12 -mt-4 rounded-full flex items-center justify-center shadow-md border-4 border-white ${recordingId === item.id ? 'bg-slate-100 text-slate-500 animate-pulse' : 'bg-slate-100 text-slate-500'}`}>{recordingId === item.id ? <StopCircle size={20}/> : <Mic size={20}/>}</button>
                           ...
                           <AnimatePresence>
                             {speechResult?.id === item.id && !isLocked && (
                               <motion.div initial={{opacity:0, height:0}} animate={{opacity:1, height:'auto'}} exit={{opacity:0, height:0}} className="bg-white mx-auto max-w-[360px] rounded-xl mt-2 p-3 shadow-sm border border-slate-100">
                                  <div className="flex justify-between items-center mb-2"><span className="text-[10px] font-bold text-slate-400">评分</span><span className={`text-xs font-black ${speechResult.data.accuracy > 0.8 ? 'text-green-500' : 'text-red-500'}`}>{Math.round(speechResult.data.accuracy * 100)}%</span></div>
                                  <div className="flex flex-wrap gap-2 justify-center">{speechResult.data.comparison.map((r, idx) => (<div key={idx} className="flex flex-col items-center"><span className={`text-xs font-mono font-bold ${r.isMatch ? 'text-slate-800' : 'text-red-500'}`}>{r.userPy || '?'}</span><span className="text-[10px] text-slate-400">{r.targetChar}</span></div>))}</div>
                               </motion.div>
                             )}
                           </AnimatePresence>
                        </motion.div>
               ...
               {visibleCount < phrases.length && <div className="text-center py-4 text-xs text-slate-400">加载更多...</div>}
            </div>
        </div>
      )}
        ...
        {showVip && (
          <div className="fixed inset-0 z-[3000] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-md">
             <motion.div initial={{scale:0.9, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0.9, opacity:0}} className="relative bg-white rounded-[2rem] p-8 w-full max-w-xs text-center shadow-2xl">
                 ...
                 <div className="w-16 h-16 mx-auto bg-gradient-to-tr from-yellow-400 to-orange-500 rounded-full flex items-center justify-center mb-4 text-white shadow-lg ring-4 ring-orange-100"><Crown size={32} fill="currentColor" /></div>
                 ...
                 <a href="https://m.me/61575187883357" target="_blank" className="block w-full py-3.5 bg-slate-900 text-white rounded-xl font-bold shadow-lg active:scale-95 transition-transform hover:bg-slate-800">联系老师激活 (30,000 Ks)</a>
             </motion.div>
          </div>
        )}
      </AnimatePresence>
