...
const DEFAULT_CONFIG = {
  ...
  modelId: 'meta/llama-3.1-70b-instruct', // 默认使用速度更快的模型
...
export default function AIChatDock({ contextData, ttsPlay }) {
  ...
  const handleSend = async () => {
    ...
    try {
      // 固定请求我们自己的后端代理 /api/chat
      const response = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          messages: apiMessages,
          config: { // 将用户在前端设置的配置传给后端
      ...
      if (!response.ok) {
        ...
        throw new Error(`服务器错误 (${response.status}): ${errorDetails.slice(0, 200)}`);
      }

      const data = JSON.parse(rawText);
      const reply = data?.choices?.?.message?.content;
      if (!reply) throw new Error("AI 返回了空内容，请检查 Key 或模型 ID 是否正确");
  ...
  return (
    <>
      ...
      <div style={{...styles.chatBox, height: expanded ? '75vh' : '60px'}}>
        ...
        {expanded && (
          <div ref={historyRef} style={styles.chatHistory}>
             ...
             {loading && (
               <div style={{alignSelf:'flex-start', background:'#fff', padding:'10px 14px', borderRadius:'12px', color:'#94a3b8', fontSize:'0.9rem', boxShadow: '0 2px 5px rgba(0,0,0,0.05)'}}>
                 AI 正在思考...
               </div>
             )}
          </div>
      ...
      <style jsx global>{`
        ...
        .markdown-body h1, .markdown-body h2, .markdown-body h3 { font-weight: bold; margin-top: 1em; margin-bottom: 0.5em; border-bottom: 1px solid #eee; padding-bottom: 0.3em;}
