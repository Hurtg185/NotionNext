...
export default function AIChatDock() {
  ...
  useEffect(() => {
    if (isAiOpen) {
      // 当 AI 打开时，向浏览器历史压入一个状态
      // 这样用户按返回键时，会触发 popstate 而不是跳转页面
...
请按你的“互动题补课规则”进行提示。
  ...
  // 模拟返回操作（用于顶部导航栏的返回按钮）
  const handleBack = () => {
    // 调用 history.back() 会触发 popstate 事件，从而关闭 AI 窗口
    window.history.back();
  };
  ...
  return (
    <>
        ...
        <div style={styles.fullScreenContainer}>
            ...
            <div style={styles.footer}>
                {!loading && suggestions.length > 0 && (
                    <div style={styles.scrollSuggestionContainer} className="hide-scrollbar">
                        {suggestions.map((s, idx) => (
                            <button 
                                key={idx} 
                                onClick={() => handleSend(s)} 
                                style={styles.scrollSuggestionBtn}
                                onMouseDown={(e) => e.currentTarget.style.transform = 'scale(0.96)'}
                                onMouseUp={(e) => e.currentTarget.style.transform = 'scale(1)'}
                                onTouchStart={(e) => e.currentTarget.style.transform = 'scale(0.96)'}
                                onTouchEnd={(e) => e.currentTarget.style.transform = 'scale(1)'}
                            >
                                <FaLightbulb color="#4f46e5" size={12} style={{marginRight:6}}/>{s}
                            </button>
                        ))}
                    </div>
                )}
      ...
      <style jsx global>{`
        ...
        /* AI 回答富文本样式增强 */
        .notion-md { font-family: -apple-system, system-ui, sans-serif; color: #334155; line-height: 1.8; }
        
        /* 列表层级增强 */
        .notion-md ul { padding-left: 1.2em; list-style-type: disc; margin: 0.5em 0; }
        .notion-md ul ul { list-style-type: circle; margin-top: 2px; }
        .notion-md ul ul ul { list-style-type: square; margin-top: 2px; }
        .notion-md ol { padding-left: 1.2em; list-style-type: decimal; margin: 0.5em 0; }
        .notion-md ol ol { list-style-type: lower-alpha; }
        
        .notion-md li { position: relative; padding-left: 0.2em; margin-bottom: 6px; }
...
const styles = {
  ...
  // --- 富文本排版样式增强 ---
  h1: { fontSize: '1.4em', fontWeight: 800, margin: '1em 0 0.5em 0', color:'#1e293b', lineHeight:1.3 },
  h2: { fontSize: '1.2em', fontWeight: 700, margin: '0.9em 0 0.5em 0', borderBottom:'2px solid #e2e8f0', paddingBottom:6, color:'#334155' },
  h3: { fontSize: '1.1em', fontWeight: 600, margin: '0.8em 0 0.4em 0', color:'#475569' },
  p: { margin: '0 0 10px 0', color: '#334155', lineHeight: '1.75' },
  strong: { fontWeight: 700, color: '#4338ca' }, // 重点字加深蓝颜色
  ul: { paddingLeft: '1.4em', marginBottom: '10px' }, 
  ol: { paddingLeft: '1.4em', marginBottom: '10px' },
  li: { marginBottom: '6px' },
  blockquote: { borderLeft: '4px solid #6366f1', background: '#f8fafc', padding: '8px 12px', borderRadius: '4px', margin: '8px 0', color: '#64748b' },
  del: { textDecoration: 'line-through', color: '#ef4444', opacity: 0.7 },
  table: { width: '100%', borderCollapse: 'collapse', margin: '12px 0', fontSize: '0.9em', boxShadow: '0 0 0 1px #e2e8f0', borderRadius: 4, overflow: 'hidden' },
  th: { border: '1px solid #cbd5e1', padding: '8px 12px', background: '#f1f5f9', fontWeight: '700', textAlign: 'left', color: '#334155' },
  td: { border: '1px solid #e2e8f0', padding: '8px 12px', verticalAlign: 'top', color: '#475569' },
  
  // ✅ 修改：追问建议容器 (横向滚动胶囊)
  scrollSuggestionContainer: { 
      display: 'flex', flexWrap: 'nowrap', gap: 10, padding: '12px 16px', overflowX: 'auto', 
      alignItems: 'center', width: '100%',
      WebkitOverflowScrolling: 'touch', msOverflowStyle: 'none'
  },
  // ✅ 修改：单个建议按钮 (气囊样式)
  scrollSuggestionBtn: { 
      flexShrink: 0, background: '#ffffff', border: '1px solid #e0e7ff', borderRadius: '24px', 
      padding: '8px 16px', fontSize: '0.85rem', color: '#4f46e5', cursor: 'pointer',
      display: 'flex', alignItems: 'center', boxShadow: '0 2px 6px rgba(79, 70, 229, 0.08)', 
      fontWeight: '600', whiteSpace: 'nowrap', transition: 'transform 0.1s'
  },
