// /components/AiChatAssistant.js - v56 (最终完整版 - 修复所有已知Bug，强化防御，优化UI，解决FB语音问题)
...
const DEFAULT_SETTINGS = {
    apiKey: '', selectedModel: 'gemini-2.5-flash', chatModels: CHAT_MODELS_LIST,
    temperature: 0.8, maxOutputTokens: 2048, contextLength: 512, // 上下文长度
...
// 模型选择器 (ModelSelector) 全屏组件
const ModelSelector = ({ settings, onSelect, onClose }) => (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-20 flex flex-col p-4 animate-fade-in" onClick={onClose}>
        <div className="w-full max-w-md m-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b dark:border-gray-700 text-center relative">
                <h3 className="text-lg font-bold">切换模型</h3>
            ...
            <div className="p-2 overflow-y-auto max-h-[60vh]">
                {(settings.chatModels || []).map(m => (
                    <button key={m.value} type="button" onClick={() => { onSelect(m.value); onClose(); }} className={`w-full text-left px-4 py-3 text-sm rounded-lg hover:bg-primary/10 ${settings.selectedModel === m.value ? 'text-primary font-bold bg-primary/10' : ''}`}>{m.name}</button>
                ))}
            </div>
        </div>
    </div>
...
// 助理选择器 (AssistantSelector) 保持不变，已是全屏
...
const AiChatAssistant = ({ onClose, isFullScreenMode = false }) => {
    ...
    const [showModelSelector, setShowModelSelector] = useState(false); // 控制 ModelSelector 模态框的显示
    ...
    const handleSubmit = async (isRegenerate = false) => {
        ...
        if (isRegenerate) {
            ...
            const updatedMessages = [...messagesForApi, userMessage]; // 使用 messagesForApi 而不是 currentConversation.messages
