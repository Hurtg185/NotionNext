 ...
 * @param {number} [props.rate=0.8] - 朗读语速 (0.1 - 10, 默认 1)。
...
const TextToSpeechButton = ({ text, lang = 'zh-CN', rate = 1, pitch = 1, voiceName }) => {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={isDisabled}
      className={`ml-2 p-1 rounded-full text-primary dark:text-secondary hover:bg-primary/[0.1] dark:hover:bg-secondary/[0.1] transition-colors duration-200
