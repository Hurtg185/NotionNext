...
const AiTtsButton = ({ text, lang, apiKey, ttsSettings, className = '' }) => {
  ...
  const synthesizeSpeech = useCallback(async (textToSpeak) => {
    ...
    const {
      ...
      googleVoiceName = 'cmn-CN-Wavenet-A', googlePitch = 0, googleRate = 1,
      externalVoice = 'zh-CN-XiaochenMultilingualNeural', externalRate = '-20%', externalPitch = '0%'
    } = ttsSettings;

    try {
      let audioBlob;
      if (selectedTtsEngine === TTS_ENGINE.GOOGLE_GENAI) {
        if (!apiKey.trim()) throw new Error('请在设置中提供 Google API 密钥。');
        const url = `https://texttospeech.googleapis.com/v1beta1/text:synthesize?key=${apiKey}`;
        const body = { input: { text: textToSpeak }, voice: { languageCode: lang, name: googleVoiceName }, audioConfig: { audioEncoding: 'MP3', speakingRate: googleRate, pitch: googlePitch } };
        ...
        } else { throw new Error('浏览器不支持系统 TTS。'); }
      }
  ...
  return (
    ...
    >
      <button disabled={isLoading} className={`tts-button inline-flex items-center justify-center rounded-full w-8 h-8 sm:w-10 sm:h-10 text-white shadow-sm transition-all duration-200 ${isLoading ? 'bg-gray-400' : 'bg-blue-500 hover:bg-blue-600'} ${className}`} aria-label={`朗读: ${text}`}>
        {isLoading ? (<svg className="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>) : (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M15.54 8.46a5 5 0 0 1 0 7.07"></path></svg>)}
      </button>
    </div>
  );
};
