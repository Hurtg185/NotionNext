// /components/ExternalTextToSpeech.js - 使用外部 API 的语音朗读组件 (晓辰版)

import React, { useState, useEffect, useRef, useCallback } from 'react';

/**
 * 使用外部 TTS API (t.leftsite.cn) 进行文本朗读的按钮组件。
 ...
 */
const ExternalTextToSpeechButton = ({
  text,
  lang = 'zh-CN',
  ...
  const synthesizeSpeech = useCallback(async (textToSpeak) => {
    ...
    // 如果 API 密钥存在，通常会作为参数或 header，但根据你的脚本，这个API不需要密钥
    // if (apiConfig.current.apiKey) url += `&api_key=${apiConfig.current.apiKey}`; // 如果未来需要，可以启用
    
    try {
      const response = await fetch(url); // 前端直接发起 fetch 请求
  ...
  }, [lang]); // 朗读语言通常也影响 API 请求，但你的 API 配置中未包含 lang，这里保留
...
export default ExternalTextToSpeechButton;
