...
const RandomImageCard = ({ banners, linkUrl, alt }) => {
    const randomImage = useMemo(() => {
        if (!banners || banners.length === 0) { console.warn("XUANCHUAN_BANNERS is empty or undefined. Using default fallback image."); return '/images/default-xuanchuan.jpg'; }
...
const LayoutIndex = props => {
  ...
  const modalContent = {
    ...
    grammar: {
      title: '语法学习',
      intro: '掌握中文语法结构，轻松构建句子！',
      children: ['基础句型', '动词时态', '量词用法', '介词短语', '比较句', '被动句', '疑问句', '复杂句式', '语法练习', '语法视频教程'].map(item => <SmartLink key={item} href='#' className='block p-2 text-center rounded bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600'>{item}</SmartLink>)
    },
    exercises: {
      title: 'HSK练习题',
      intro: '巩固所学知识，挑战不同难度的 HSK 练习题！',
      children: ['HSK1', 'HSK2', 'HSK3', 'HSK4', 'HSK5', 'HSK6'].map(item => <SmartLink key={item} href='#' className='block p-2 text-center rounded bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600'>{`${item} 练习`}</SmartLink>)
    },
    vocabulary: {
      title: 'HSK生词',
      intro: '按 HSK 等级分类的词汇表，助你高效记忆！',
      children: ['HSK1', 'HSK2', 'HSK3', 'HSK4', 'HSK5', 'HSK6'].map(item => <SmartLink key={item} href='#' className='block p-2 text-center rounded bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600'>{`${item} 生词`}</SmartLink>)
    },
    phrases: {
      title: '常用短语',
      intro: '掌握常用中文短语，轻松应对日常交流！',
      children: ['打招呼', '购物', '问路', '点餐', '交通', '看医生', '银行', '酒店入住', '紧急情况', '日常对话'].map(item => <SmartLink key={item} href='#' className='block p-2 text-center rounded bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600'>{item}</SmartLink>)
    },
    pinyin: {
      title: '拼音学习',
      intro: '快速掌握中文发音，从零开始学拼音！',
      children: ['拼音基础发音', '声母韵母表', '四声调学习', '拼音易错点', '拼音发音视频', '声调练习', '拼音输入法', '拼音口诀', '拼音学习App推荐', '免费拼音课程'].map(item => <SmartLink key={item} href='#' className='block p-2 text-center rounded bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600'>{item}</SmartLink>)
    }
  }
