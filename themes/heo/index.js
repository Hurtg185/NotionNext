// themes/heo/index.js (最终正确版本：渲染内部组件)
...
// ==================================================================
// 步骤 1: 在这里导入您自己的AI助手组件
// ==================================================================
// 例如: import YourAIAssistantComponent from '@/components/AI/ChatBox'
// 请将下面的占位符替换成您真实的组件导入语句
import AiChatAssistant from '@/components/AiChatAssistant' // <-- 修改这里
// ==================================================================
...
// --- 用于嵌入您自己AI助手的全屏容器 (无改动) ---
const AIAssistantModal = ({ isOpen, onClose, children }) => {
  if (!isOpen) return null
  return (
    <div className='fixed inset-0 bg-white dark:bg-gray-900 z-[100] flex flex-col animate-fade-in'>
      <header className='flex items-center justify-between p-4 border-b dark:border-gray-700 shadow-sm flex-shrink-0 bg-white dark:bg-gray-800'>
...
const LayoutBase = props => {
  // ... (代码无变化，保持原样)
  const { children, slotTop, className, post } = props; const { fullWidth, isDarkMode } = useGlobal(); const router = useRouter(); const isIndex = router.pathname === '/'; const isSlugPage = post && post.slug; const headerSlot = (<header><Header {...props} />{isIndex ? (<><NoticeBar /><Hero {...props} /></>) : null}{isSlugPage && !fullWidth ? <PostHeader {...props} isDarkMode={isDarkMode} /> : null}</header>); const slotRight = router.route === '/404' || fullWidth ? null : <SideRight {...props} />; const maxWidth = fullWidth ? 'max-w-[96rem] mx-auto' : 'max-w-[86rem]'; const HEO_HERO_BODY_REVERSE = siteConfig('HEO_HERO_BODY_REVERSE', false, CONFIG); useEffect(() => { loadWowJS() }, []); return (<div id='theme-heo' className={`${siteConfig('FONT_STYLE')} bg-[#f7f9fe] dark:bg-[#18171d] h-full min-h-screen flex flex-col scroll-smooth`}><Style />{headerSlot}<main id='wrapper-outer' className={`w-full ${maxWidth} mx-auto relative md:px-5`}><div id='container-inner' className={`${HEO_HERO_BODY_REVERSE ? 'flex-row-reverse' : ''} w-full mx-auto lg:flex justify-center relative z-10`}><div className={`w-full h-auto ${className || ''}`}>{slotTop}{children}</div><div className='lg:px-2'></div><div className='hidden xl:block'>{slotRight}</div></div></main><Footer /></div>)
}

// --- 功能按钮 (无改动) ---
const FunctionButton = ({ title, icon, onClick, href, img, target }) => {
    const style = img ? { backgroundImage: `url(${img})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}; const iconColorClass = img ? 'text-white' : 'text-gray-500 dark:text-gray-300 group-hover:text-indigo-500 dark:group-hover:text-yellow-500'; const textColorClass = img ? 'text-white' : 'text-gray-700 dark:text-gray-200'; const content = (<>{img && <div className="absolute inset-0 bg-black bg-opacity-40 rounded-xl"></div>}<div className='relative z-10 flex flex-col items-center justify-center'><div className={`text-3xl ${iconColorClass} transition-colors duration-200`}><i className={icon} /></div><div className={`mt-2 text-sm font-semibold ${textColorClass}`}>{title}</div></div></>); if (href) { return (<SmartLink href={href} target={target} className='group relative flex flex-col justify-center items-center w-full h-24 bg-white dark:bg-[#1e1e1e] border dark:border-gray-700 rounded-xl shadow-md transform hover:scale-105 transition-transform duration-200 overflow-hidden' style={style}>{content}</SmartLink>)} return (<button onClick={onClick} className='group relative flex flex-col justify-center items-center w-full h-24 bg-white dark:bg-[#1e1e1e] border dark:border-gray-700 rounded-xl shadow-md transform hover:scale-105 transition-transform duration-200 overflow-hidden' style={style}>{content}</button>)
}
...
const QuickAccessGrid = ({ setActiveModal }) => {
    const functions = [{ title: '报名课程', icon: 'fa-solid fa-graduation-cap', modal: 'enroll' },{ title: '找工作', icon: 'fa-solid fa-briefcase', modal: 'jobs' },{ title: '试看课程', icon: 'fa-solid fa-video', modal: 'trial' }]; return (<div className='py-2'><div className='grid grid-cols-3 gap-4'>{functions.map(func => <FunctionButton key={func.title} title={func.title} icon={func.icon} onClick={() => setActiveModal(func.modal)} />)}</div></div>)
}

// --- 提问交流模块 (无改动) ---
const AskQuestionModule = () => {
    const facebookGroupUrl = "https://www.facebook.com/share/g/15Fh7mrpa8/"; return (<div className='py-4'><a href={facebookGroupUrl} target="_blank" rel="noopener noreferrer" className='group flex items-center justify-between p-4 bg-white dark:bg-[#1e1e1e] border dark:border-gray-700 rounded-xl shadow-md hover:shadow-lg transition-shadow duration-200'><div className='flex items-center'><i className='fas fa-question-circle text-3xl text-indigo-500 mr-4'></i><div><h3 className='font-bold text-lg dark:text-white'>提问交流</h3><p className='text-sm text-gray-500 dark:text-gray-400'>遇到学习问题？来这里和大家一起讨论！</p></div></div><i className='fas fa-arrow-right text-gray-400 group-hover:translate-x-1 transition-transform duration-200'></i></a></div>)
}

// --- 学习工具功能区 (无改动) ---
const StudyToolsGrid = ({ setActiveModal }) => {
    const functions = [{ title: '书籍', icon: 'fa-solid fa-book', href: 'https://books.843075.xyz' },{ title: '语法', icon: 'fa-solid fa-pen-ruler', modal: 'grammar' },{ title: '练习题', icon: 'fa-solid fa-file-pen', modal: 'exercises' },{ title: '生词', icon: 'fa-solid fa-spell-check', modal: 'vocabulary' },{ 'title': '短语', 'icon': 'fas fa-comments', modal: 'phrases' },{ 'title': '拼音', 'icon': 'fas fa-font', modal: 'pinyin' }]; return (<div className='py-2'><div className='text-2xl font-bold mb-4 text-center dark:text-white'>学习工具</div><div className='grid grid-cols-3 gap-4'>{functions.map(func => <FunctionButton key={func.title} title={func.title} icon={func.icon} href={func.href} onClick={() => setActiveModal(func.modal)} />)}</div></div>)
}

// --- 随机图片卡片组件 (无改动) ---
const RandomImageCard = ({ banners, linkUrl, alt }) => {
    const randomImage = useMemo(() => { if (!banners || banners.length === 0) { console.warn("XUANCHUAN_BANNERS is empty or undefined. Using default fallback image."); return '/images/default-xuanchuan.jpg'; } return banners[Math.floor(Math.random() * banners.length)]; }, [banners]); return (<section className='mt-4'><SmartLink href={linkUrl || '#'}><div className='rounded-xl shadow-md overflow-hidden transform hover:scale-105 transition-transform duration-300'><LazyImage src={randomImage} alt={alt} className="w-full h-auto" /></div></SmartLink></section>)
}
  ...
  return (
    <>
      ...
      <AIAssistantModal isOpen={isAiAssistantOpen} onClose={() => setIsAiAssistantOpen(false)}>
        {/* ================================================================== */}
        {/* 步骤 2: 在这里使用您导入的AI助手组件                         */}
        {/* ================================================================== */}
        <AiChatAssistant /> {/* <-- 修改这里 */}
        {/* ================================================================== */}
      </AIAssistantModal>
    </>
  )
...
/**归档*/
const LayoutArchive = props => {const { archivePosts } = props;return (<div className='p-5 rounded-xl border dark:border-gray-600 max-w-6xl w-full bg-white dark:bg-[#1e1e1e]'><CategoryBar {...props} border={false} /><div className='px-3'>{Object.keys(archivePosts).map(archiveTitle => (<BlogPostArchivekey={archiveTitle}posts={archivePosts[archiveTitle]}archiveTitle={archiveTitle}/>))}</div></div>)}
/**文章详情*/
const LayoutSlug = props => {const { post, lock, validPassword } = props;const { locale, fullWidth } = useGlobal();const [hasCode, setHasCode] = useState(false);const [activeModal, setActiveModal] = useState(null);useEffect(() => {const hasCode = document.querySelectorAll('[class^="language-"]').length > 0;setHasCode(hasCode)}, []);const commentEnable =siteConfig('COMMENT_TWIKOO_ENV_ID') ||siteConfig('COMMENT_WALINE_SERVER_URL') ||siteConfig('COMMENT_VALINE_APP_ID') ||siteConfig('COMMENT_GISCUS_REPO') ||siteConfig('COMMENT_CUSDIS_APP_ID') ||siteConfig('COMMENT_UTTERRANCES_REPO') ||siteConfig('COMMENT_GITALK_CLIENT_ID') ||siteConfig('COMMENT_WEBMENTION_ENABLE');const router = useRouter();const waiting404 = siteConfig('POST_WAITING_TIME_FOR_404') * 1000;useEffect(() => {if (!post) {setTimeout(() => {if (isBrowser) {const article = document.querySelector('#article-wrapper #notion-article');if (!article) {router.push('/404').then(() => {console.warn('找不到页面', router.asPath)})}}},waiting404)}}, [post]);const modalContent = {appDownload: {title: '安卓APP下载',intro: '下载我们的专属学习APP，随时随地高效学习中文！请按照以下步骤安装：',children: (<div className='text-sm text-gray-700 dark:text-gray-300'><a href='#' target='_blank' rel='noopener noreferrer' className='block w-full text-center p-3 rounded-lg bg-blue-500 text-white font-bold mb-4 hover:bg-blue-600 transition-colors'>点击此处下载安卓APP</a><ol className='list-decimal list-inside space-y-2 text-left'><li>点击上方链接下载APP文件。</li><li>下载完成后，请断开网络（Wi-Fi和移动数据）。</li><li>找到下载的APP文件（通常在“文件管理器”或“下载”文件夹），点击安装。</li><li>如果提示“禁止安装未知来源应用”，请前往手机设置中允许安装。</li><li>安装成功后，重新连接网络即可使用。</li></ol><a href='#' target='_blank' rel='noopener noreferrer' className='block text-center mt-4 text-blue-500 hover:underline'>查看详细图文/视频教程 →</a></div>)},notifications: {title: '加入通知群',intro: '获取最新课程优惠、免费资料、招聘信息、直播通知，重要消息不遗漏！',children: (<><SmartLink href='#' target='_blank' rel='noopener noreferrer' className='flex items-center p-3 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-all duration-200'><i className='fab fa-telegram-plane text-xl mr-3'></i> <span className='font-semibold'>Telegram</span></SmartLink><SmartLink href='#' target='_blank' rel='noopener noreferrer' className='flex items-center p-3 rounded-lg bg-green-400 text-white hover:bg-green-500 transition-all duration-200'><i className='fab fa-line text-xl mr-3'></i> <span className='font-semibold'>Line</span></SmartLink><SmartLink href='#' target='_blank' rel='noopener noreferrer' className='flex items-center p-3 rounded-lg bg-purple-500 text-white hover:bg-purple-600 transition-all duration-200'><i className='fab fa-viber text-xl mr-3'></i> <span className='font-semibold'>Viber</span></SmartLink><SmartLink href='#' target='_blank' rel='noopener noreferrer' className='flex items-center p-3 rounded-lg bg-blue-700 text-white hover:bg-blue-800 transition-all duration-200'><i className='fab fa-facebook-f text-xl mr-3'></i> <span className='font-semibold'>Facebook 个人主页</span></SmartLink><SmartLink href='#' target='_blank' rel='noopener noreferrer' className='flex items-center p-3 rounded-lg bg-black text-white hover:bg-gray-800 transition-all duration-200'><i className='fab fa-tiktok text-xl mr-3'></i> <span className='font-semibold'>TikTok</span></SmartLink></>)},moreCourses: {title: '报名课程',intro: '结合中缅教学方案，高效学习中文，价格比大部分缅甸机构更优惠！请通过以下方式联系我们，获取专属学习方案：',children: (<><SmartLink href='#' target='_blank' rel='noopener noreferrer' className='flex items-center p-3 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-all duration-200'><i className='fab fa-telegram-plane text-xl mr-3'></i> <span className='font-semibold'>Telegram 联系</span></SmartLink><SmartLink href='#' target='_blank' rel='noopener noreferrer' className='flex items-center p-3 rounded-lg bg-green-400 text-white hover:bg-green-500 transition-all duration-200'><i className='fab fa-line text-xl mr-3'></i> <span className='font-semibold'>Line 联系</span></SmartLink><SmartLink href='#' target='_blank' rel='noopener noreferrer' className='flex items-center p-3 rounded-lg bg-purple-500 text-white hover:bg-purple-600 transition-all duration-200'><i className='fab fa-viber text-xl mr-3'></i> <span className='font-semibold'>Viber 联系</span></SmartLink><SmartLink href='#' target='_blank' rel='noopener noreferrer' className='flex items-center p-3 rounded-lg bg-blue-700 text-white hover:bg-blue-800 transition-all duration-200'><i className='fab fa-facebook-f text-xl mr-3'></i> <span className='font-semibold'>Facebook 个人主页</span></SmartLink></>)}};const currentModal = modalContent[activeModal];return (<><div className={`article h-full w-full ${fullWidth ? '' : 'xl:max-w-5xl'} ${hasCode ? 'xl:w-[73.15vw]' : ''}  bg-white dark:bg-[#18171d] dark:border-gray-600 lg:hover:shadow lg:border rounded-2xl lg:px-2 lg:py-4 `}>{lock && <PostLock validPassword={validPassword} />}    {!lock && post && (<div className='mx-auto md:w-full md:px-5'><article id='article-wrapper' itemScope itemType='https://schema.org/Movie'><div className='px-5'><RandomImageCard banners={XUANCHUAN_BANNERS} linkUrl="#" alt="培训班简介" /></div><section className='wow fadeInUp p-5 justify-center mx-auto' data-wow-delay='.2s'><ArticleExpirationNotice post={post} /><AISummary aiSummary={post.aiSummary} /><WWAds orientation='horizontal' className='w-full' />{post?.type === 'Post' && <PostCopyright {...props} />}{post && <NotionPage post={post} />}<WWAds orientation='horizontal' className='w-full' /></section><ShareBar post={post} /><div className='bg-white shadow-md my-2 p-4 rounded-md dark:bg-black'><div className='grid grid-cols-3 gap-3'><FunctionButton title="APP下载" icon="fas fa-mobile-alt" onClick={() => setActiveModal('appDownload')} /><FunctionButton title="通知群" icon="fas fa-bell" onClick={() => setActiveModal('notifications')} /><FunctionButton title="更多课程" icon="fas fa-chalkboard-teacher" onClick={() => setActiveModal('moreCourses')} /></div></div>{post?.type === 'Post' && (<div className='px-5'><RandomImageCard banners={XUANCHUAN_BANNERS} linkUrl="/jobs" alt="招聘信息" /></div>)}</article>{fullWidth ? null : (<div className={`${commentEnable && post ? '' : 'hidden'}`}><hr className='my-4 border-dashed' /><div className='py-2'><AdSlot /></div><div className='duration-200 overflow-x-auto px-5'><div className='text-2xl dark:text-white'><i className='fas fa-comment mr-1' />{locale.COMMON.COMMENTS}</div><Comment frontMatter={post} className='' /></div></div>)}</div>)}</div><Modal isOpen={!!activeModal} onClose={() => setActiveModal(null)} title={currentModal?.title} intro={currentModal?.intro}>{currentModal?.children}</Modal><FloatTocButton {...props} /></>)}
/**404*/
...
/**分类列表*/
const LayoutCategoryIndex = props => {const { categoryOptions } = props;const { locale } = useGlobal();return (<div id='category-outer-wrapper' className='mt-8 px-5 md:px-0'><div className='text-4xl font-extrabold dark:text-gray-200 mb-5'>{locale.COMMON.CATEGORY}</div><divid='category-list'className='duration-200 flex flex-wrap m-10 justify-center'>{categoryOptions?.map(category => {return (<SmartLinkkey={category.name}href={`/category/${category.name}`}passHreflegacyBehavior><divclassName={'group mr-5 mb-5 flex flex-nowrap items-center border bg-white text-2xl rounded-xl dark:hover:text-white px-4 cursor-pointer py-3 hover:text-white hover:bg-indigo-600 transition-all hover:scale-110 duration-150'}><HashTag className={'w-5 h-5 stroke-gray-500 stroke-2'} />{category.name}<div className='bg-[#f1f3f8] ml-1 px-2 rounded-lg group-hover:text-indigo-600 '>{category.count}</div></div></SmartLink>)})}</div></div>)}
/**标签列表*/
const LayoutTagIndex = props => {const { tagOptions } = props;const { locale } = useGlobal();return (<div id='tag-outer-wrapper' className='px-5 mt-8 md:px-0'><div className='text-4xl font-extrabold dark:text-gray-200 mb-5'>{locale.COMMON.TAGS}</div><divid='tag-list'className='duration-200 flex flex-wrap space-x-5 space-y-5 m-10 justify-center'>{tagOptions.map(tag => {return (<SmartLinkkey={tag.name}href={`/tag/${tag.name}`}passHreflegacyBehavior><divclassName={'group flex flex-nowrap items-center border bg-white text-2xl rounded-xl dark:hover:text-white px-4 cursor-pointer py-3 hover:text-white hover:bg-indigo-600 transition-all hover:scale-110 duration-150'}><HashTag className={'w-5 h-5 stroke-gray-500 stroke-2'} />{tag.name}<div className='bg-[#f1f3f8] ml-1 px-2 rounded-lg group-hover:text-indigo-600 '>{tag.count}</div></div></SmartLink>)})}</div></div>)}
