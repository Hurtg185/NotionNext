...
const GeminiSettingsModal = ({ isOpen, onClose, onSave, currentKey, currentModel }) => {
  ...
  const handleSave = () => { onSave(apiKey, model); onClose(); };
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="请输入你的 Gemini API Key" className="w-full mt-1 p-2 border rounded-md dark:bg-gray-700"/>
          ...
          <div>
            ...
            <input type="text" value={model} onChange={(e) => setModel(e.target.value)} placeholder="例如 gemini-pro" className="w-full mt-1 p-2 border rounded-md dark:bg-gray-700"/>
          </div>
        </div>
...
const PostDetailPage = () => {
  ...
  const handleTranslate = async (text, type, id, forceOriginal = false) => {
    ...
    try { const translatedText = await callGeminiApi(text, geminiApiKey, geminiModel); if (type === 'post') setTranslatedPostContent(translatedText); if (type === 'comment') setTranslatedComments(prev => ({ ...prev, [id]: translatedText })); } catch (error) { console.error('翻译错误:', error); alert(`翻译失败: ${error.message}`); } finally { setIsTranslating(false); }
  };
  ...
  const postIsLiked = user && Array.isArray(post.likes) && post.likes.includes(user.uid);
  const postIsDisliked = user && Array.isArray(post.dislikes) && post.dislikes.includes(user.uid);
