// pages/forum/post/[id].js (最终完整版 - 修复所有功能和UI)
...
const CommentItem = ({ comment, allComments, user, postAuthorId, handleVote, handleDelete, handleReply }) => {
  ...
  const visibleReplies = directReplies.slice(0, 2); // 折叠时显示2条
          ...
          <div className="mt-3 bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg space-y-2">
            ...
            {hasMoreReplies && (
                <Link href={`/forum/comment/${comment.id}`} passHref>
                    <a className="text-blue-500 dark:text-blue-400 text-sm font-semibold hover:underline mt-2 inline-block">
                        查看全部 {directReplies.length} 条回复
                    </a>
                </Link>
            )}
          </div>
...
const PostDetailPage = () => {
  ...
  const voteHandler = async (docRef, type, currentLikes, currentDislikes, isPost = false) => {
    ...
    if (type === 'like') {
        if (isLiked) { batch.update(docRef, { likes: arrayRemove(userId) }); if(isPost) batch.update(docRef, { likesCount: increment(-1) }); }
        else { batch.update(docRef, { likes: arrayUnion(userId) }); if(isPost) batch.update(docRef, { likesCount: increment(1) }); if (isDisliked) { batch.update(docRef, { dislikes: arrayRemove(userId) }); } }
    } else {
        if (isDisliked) { batch.update(docRef, { dislikes: arrayRemove(userId) }); }
        else { batch.update(docRef, { dislikes: arrayUnion(userId) }); if (isLiked) { batch.update(docRef, { likes: arrayRemove(userId) }); if(isPost) batch.update(docRef, { likesCount: increment(-1) }); } }
    }
