// pages/forum/post/[id].js (终极美化版 - 贴吧风格盖楼)
...
// 【新增】用于显示折叠回复的紧凑组件
const CompactReply = ({ reply, getParentAuthorName }) => (
  <div className="text-sm text-gray-700 dark:text-gray-300">
    ...
    <Link href={`/profile/${reply.parentId ? allComments.find(c => c.id === reply.parentId)?.authorId : ''}`} passHref>
        <a className="font-semibold text-gray-800 dark:text-white hover:underline">@{getParentAuthorName(reply.parentId)}</a>
    </Link>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400 mt-2">
          ...
          <div className="flex items-center space-x-5">
            ...
            <button onClick={() => handleReply(comment)} className="flex items-center space-x-1 text-base text-gray-400 dark:text-gray-500 hover:text-blue-500 transition-colors"><i className="fas fa-comment-dots text-xl"></i><span className="font-semibold">回复</span></button>
                ...
                <div className="space-y-1">
                    {visibleReplies.map(reply => (
                        <CompactReply 
                            key={reply.id} 
                            reply={reply} 
                            getParentAuthorName={getParentAuthorName} 
                        />
                    ))}
                </div>
            )}
