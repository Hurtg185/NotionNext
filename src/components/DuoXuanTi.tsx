...
import { DuoXuanQuizData, DuoXuanFormFields, UserAnswer } from '../types/quiz';
import { speakText, getPinyin, checkAnswerCorrectness } from '../utils/quizUtils';

// Zod schema for multiple choice form validation
const duoXuanSchema = z.object({
  selectedOptions: z.array(z.string()).min(1, '请至少选择一个答案'),
});

interface DuoXuanTiProps {
  quizData: DuoXuanQuizData;
...
const DuoXuanTi: React.FC<DuoXuanTiProps> = ({
  quizData,
  onAnswerSubmit,
  isReviewMode = false,
  userAnswer = null,
}) => {
  const { control, handleSubmit, formState: { errors, isValid }, watch, setValue } = useForm<DuoXuanFormFields>({
    resolver: zodResolver(duoXuanSchema),
    defaultValues: {
      selectedOptions: (isReviewMode && Array.isArray(userAnswer)) ? userAnswer : [],
    },
    mode: 'onChange',
  });
  ...
  useEffect(() => {
    ...
    if (isReviewMode && Array.isArray(userAnswer)) {
      setValue('selectedOptions', userAnswer);
    } else if (!isReviewMode) {
      setValue('selectedOptions', []);
    }

    if (isReviewMode && quizData.correctAnswers) {
      const correct = checkAnswerCorrectness(quizData.type, userAnswer || [], quizData.correctAnswers);
      setIsCorrectFeedback(correct);
    } else {
      setIsCorrectFeedback(null);
    }
  }, [quizData.id, isReviewMode, userAnswer, setValue, quizData.correctAnswers, quizData.type]);
  ...
  const onSubmit = (data: DuoXuanFormFields) => {
    if (isReviewMode) return;
    const { selectedOptions } = data;
    const isCorrect = checkAnswerCorrectness(quizData.type, selectedOptions, quizData.correctAnswers);
    setIsCorrectFeedback(isCorrect);
    setIsSubmitted(true);
    onAnswerSubmit(quizData.id, selectedOptions, isCorrect);
  };
  ...
  const handleOptionClick = (optionId: string) => {
    ...
    if (currentSelection.includes(optionId)) {
      setValue('selectedOptions', currentSelection.filter(id => id !== optionId), { shouldValidate: true });
    } else {
      setValue('selectedOptions', [...currentSelection, optionId], { shouldValidate: true });
    }
  };

  const getOptionClassName = (optionId: string) => {
    let className = 'flex items-center p-3 border rounded-lg cursor-pointer transition-colors duration-200 ease-in-out';
    const isOptionCurrentlySelected = selectedOptions.includes(optionId);
    const isCorrectOption = quizData.correctAnswers?.includes(optionId);
    const isUserAnsweredInReview = Array.isArray(userAnswer) && userAnswer.includes(optionId);

    if (isReviewMode) {
      if (isCorrectOption) {
        className += ' bg-green-100 border-green-500 text-green-700';
      } else if (isUserAnsweredInReview && !isCorrectOption) {
      ...
      if (isSubmitted) {
        if (isCorrectOption) {
          className = className.replace(/bg-blue-50|border-blue-500|text-blue-700/, '');
          className += ' bg-green-100 border-green-500 text-green-700';
        } else if (isOptionCurrentlySelected && !isCorrectOption) {
          className = className.replace(/bg-blue-50|border-blue-500|text-blue-700/, '');
          className += ' bg-red-100 border-red-500 text-red-700';
        }
      }
              ...
              <Controller
                ...
                render={({ field }) => (
                  <input
                    ...
                    onChange={() => { /* Handled by label's onClick */ }}
